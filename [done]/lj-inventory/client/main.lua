...
RegisterNetEvent('xt-inventory:client:dong', function()
    closeInventory()
end)
...
RegisterNetEvent('inventory:client:set:busy', function(bool)
...
--[[ RegisterNetEvent('inventory:client:CraftItems', function(itemName, itemCosts, amount, toSlot, points)
    ...
    isCrafting = true
    QBCore.Functions.Progressbar("repair_vehicle", "Chế tạo..", (math.random(2000, 5000) * amount), false, true, {
...
RegisterNetEvent('inventory:client:PickupSnowballs', function()
    ...
    QBCore.Functions.Progressbar("pickupsnowball", "Lấy tuyết", 1500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent("inventory:client:UseTool", function(toolData)
    local ped = PlayerPedId()
    local toolName = tostring(toolData.name)
    if toolName == "tool_fishingrod" then
        TriggerEvent('qg-fishing:client:use:fishingrod')
        TriggerEvent('xt-doben:client:set:current:tool', toolData)
    elseif toolName == "tool_drill_2" then
        TriggerEvent('xt-daoda:client:use:drill')
        TriggerEvent('xt-doben:client:set:current:tool', toolData)
    elseif toolName == "tool_scissor" then
        TriggerEvent('qg-tailor:client:use:scissor')
        TriggerEvent('xt-doben:client:set:current:tool', toolData)
    elseif toolName == "tool_scissor_2" then
        TriggerEvent('xt-cansa:client:use:scissor')
        TriggerEvent('xt-doben:client:set:current:tool', toolData)
    elseif toolName == "tool_axe" then
        TriggerEvent('xt-tieuphu:client:use:axe')
        TriggerEvent('xt-doben:client:set:current:tool', toolData)
    elseif toolName == "tool_bucket" then
        TriggerEvent('xt-vatsua:client:use:bucket')
        TriggerEvent('xt-doben:client:set:current:tool', toolData)
    end
end)
...
RegisterCommand('inventory', function()
    if not isCrafting and not inInventory then
        if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() and not Config.InventoryBusy then
            local ped = PlayerPedId()
            local curVeh = nil
            ...
            if IsPedInAnyVehicle(ped) then -- Is Player In Vehicle
                ...
                if vehicle ~= 0 and vehicle ~= nil then
                    ...
                    if #(pos - trunkpos) < 2.0 and not IsPedInAnyVehicle(ped) then
                        if GetVehicleDoorLockStatus(vehicle) < 2 then
                            ...
                            exports['xt-notify']:Alert("THÔNG BÁO", "Phương tiện đã khoá!", 5000, 'error')
               ...
               if CurrentVehicle then
                ...
                local closestVehicle = QBCore.Functions.GetClosestVehicle(GetEntityCoords(ped))
                local maxweight = 0
                local slots = 0
                local xe = GetEntityModel(closestVehicle)
                if xe == `rancherxl` or xe == `yosemite` or xe == `dubsta3` or xe == `slamvan3` or xe == `rebel2` then
                        maxweight = 150000
                        slots = 30
				elseif xe == `freecrawler` or xe == `brawler` or xe == `caracara2` then 
						maxweight = 200000
						slots = 30
                elseif vehicleClass == 0 then
                    maxweight = 38000
                    slots = 30
            ...
            elseif VendingMachine then
                local ShopItems = {}
                ShopItems.label = "Máy bán hàng tự động"
                ShopItems.items = Config.VendingItem
                ShopItems.slots = #Config.VendingItem
                TriggerServerEvent("inventory:server:OpenInventory", "shop", "Máy_"..math.random(1, 99), ShopItems)
            else
                openAnim()
                TriggerServerEvent("inventory:server:OpenInventory")
            end
        elseif Config.InventoryBusy then
            exports['xt-notify']:Alert("HỆ THỐNG", "Sống chậm lại thôi", 5000, 'error')
        end
    end
end)

RegisterKeyMapping('inventory', 'Mở kho đồ', 'keyboard', 'TAB')
...
RegisterKeyMapping('hotbar', 'Hiện thanh vật phẩm nhanh', 'keyboard', 'z')
...
for i = 1, 6 do
    ...
    RegisterKeyMapping('slot' .. i, 'Sử dụng vật phẩm số ' .. i, 'keyboard', i)
end
...
RegisterNUICallback("GiveItem", function(data)
    ...
    if player ~= -1 and distance < 3 then
        if (data.inventory == 'player') then
            ...
            exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không sở hữu vật phẩm!", 5000, 'error')
        end
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Không có ai ở cạnh!", 5000, 'error')
    end
end)
