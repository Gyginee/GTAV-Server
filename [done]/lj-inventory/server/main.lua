...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
	...
	if Player.Functions.RemoveItem(itemData.name, itemAmount, itemData.slot) then
		...
		TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", 'Bạn không có vật phẩm này', 5000, 'error')
		return
	end
end
	...
	if not ply.state.inv_busy then
		if name and id then
			...
			elseif name == "glovebox" then
				...
				secondInv.label = "Cốp trong-"..id
				secondInv.maxweight = 10000
				secondInv.inventory = {}
				secondInv.slots = 5
				if Gloveboxes[id] and Gloveboxes[id].isOpen then
					secondInv.name = "none-inv"
					secondInv.label = "Cốp trong-Trống"
					secondInv.maxweight = 10000
					secondInv.inventory = {}
					secondInv.slots = 0
				else
			...
			elseif name == "otherplayer" then
				...
				if Drops[id] and not Drops[id].isOpen then
					secondInv.name = id
					secondInv.label = "Mặt đất-"..tostring(id)
		...
		TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", 'Không thể tiếp cận', 5000, 'error')
	end
end)
...
RegisterNetEvent('inventory:server:UseItem', function(inventory, item)
	...
	if inventory == "player" or inventory == "hotbar" then
		...
		if inventory == "hotbar" then
			if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "ammo" then
			end
		end
		if itemData then
			TriggerClientEvent("QBCore:Client:UseItem", src, itemData)
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						TriggerClientEvent('xt-inventory:client:dong', src)
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				...
				if IsItemValid then
					...
					TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", 'Bạn không thể bán vật phẩm', 5000, 'error')
				end
			else
				-- drop
			...
			TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", 'Vật phẩm không tồn tại', 5000, 'error')
		end
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		if QBCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", itemInfo["label"] .. " đã được mua!", 5000, 'success')
					TriggerEvent("qb-log:server:CreateLog", "dealers", "**MUA TỪ DEALER**", "green", "**"..GetPlayerName(src) .. "** đã mua " .. itemInfo["label"] .. " với giá $"..price)
				else
		...
		elseif QBCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
			if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
				...
				TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", itemInfo["label"] .. " đã được mua!", 5000, 'success')
				TriggerEvent("qb-log:server:CreateLog", "shops", "**ĐÃ MUA**", "green", "**"..GetPlayerName(src) .. "** đã mua " .. itemInfo["label"] .. " với giá $"..price)
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent('xt-notify:client:Alert',src, 'Bạn không có vật phẩm yêu cầu', 5000, 'error')
		end
	elseif fromInventory == "attachment_crafting" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("qb-log:server:CreateLog", "drop", "**ĐỔI VẬT PHẨM**", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) đổi vật phẩm; tên: **"..toItemData.name.."**, số lượng: **" .. toAmount .. "** với vật phẩm; name: **"..fromItemData.name.."**, số lượng: **" .. fromAmount .. "** - dropid: *" .. fromInventory .. "*")
					else
						TriggerEvent("qb-log:server:CreateLog", "drop", "**CHỒNG VẬT PHẨM**", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) chồng vật phẩm; tên: **"..toItemData.name.."**, số lượng: **" .. toAmount .. "** - từ dropid: *" .. fromInventory .. "*")
					end
				else
					TriggerEvent("qb-log:server:CreateLog", "drop", "**NHẬN VẬT PHẨM**", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) nhận vật phẩm; tên: **"..fromItemData.name.."**, số lượng: **" .. fromAmount.. "** -  dropid: *" .. fromInventory .. "*")
				end
				Player.Functions.AddItem(fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				toInventory = tonumber(toInventory)
			...
			TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO",' Vật phẩm không tồn tại', 'error')
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
	...
	if Player == OtherPlayer then return TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", 'Bạn không thể gửi cho bản thân', 5000, 'error') end
	if dist > 2 then return TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", 'Quá xa để gửi vật phẩm', 5000, 'error')end
	local item = Player.Functions.GetItemBySlot(slot)
	if not item then TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", 'Vật phẩm không tìm thấy', 5000, 'error'); return end
	if item.name ~= name then TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", 'Không tìm thấy vật phẩm đúng', 5000, 'error'); return end
	if item.type ~= "tool" or item.type ~= "weapon" then
		if amount <= item.amount then
			...
			if Player.Functions.RemoveItem(item.name, amount, item.slot) then
				if OtherPlayer.Functions.AddItem(item.name, amount, false, item.info) then
					...
					TriggerClientEvent('xt-notify:client:Alert', target,"THÔNG BÁO","THÔNG BÁO", "Bạn đã nhận "..amount..' '..item.label.." từ "..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname, 5000, 'success')
					TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
					TriggerClientEvent('inventory:client:ItemBox',src, QBCore.Shared.Items[item.name], "remove", amount)
					TriggerClientEvent('xt-notify:client:Alert',src, "Bạn đã gửi cho " .. OtherPlayer.PlayerData.charinfo.firstname.." "..OtherPlayer.PlayerData.charinfo.lastname.. " " .. amount .. " " .. item.label .."!", 5000, 'success')
					TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
					TriggerClientEvent('qb-inventory:client:giveAnim', src)
					TriggerClientEvent('qb-inventory:client:giveAnim', target)
				else
					Player.Functions.AddItem(item.name, amount, item.slot, item.info)
					TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", 'Kho đồ của người chơi đã đầy', 5000, 'error')
					TriggerClientEvent('xt-notify:client:Alert', target,"THÔNG BÁO", 'Kho đồ đã đầy', 5000, 'error')
					TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
					TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, false)
				end
			else
				TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", 'Không đủ vật phẩm', 5000, 'error')
			end
		else
...
QBCore.Commands.Add("resetinv", "Xoá kho đồ (Admin Only)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID of stash or license plate"}}, true, function(source, args)
	...
	if invType ~= nil and invId ~= nil then
		...
		elseif invType == "stash" then
			...
			TriggerClientEvent('xt-notify:client:Alert', source,"THÔNG BÁO", 'Định dạng không hợp lệ', 5000, 'error')
		end
	else
		TriggerClientEvent('xt-notify:client:Alert', source,"THÔNG BÁO", 'Trường điền không đúng', 5000, 'error')
	end
end, "admin")

QBCore.Commands.Add("rob", "Cướp người chơi", {}, false, function(source, args)
	TriggerClientEvent("police:client:RobPlayer", source)
end)

QBCore.Commands.Add("giveitem", "Lấy vật phẩm (Admin Only)", {{name="id", help="ID"},{name="item", help="Tên vật phẩm"}, {name="amount", help="Số lượng"}}, true, function(source, args)
		...
		if amount > 0 then
			if itemData then
				...
				elseif itemData["name"] == "markedbills" then
					info.worth = math.random(Config.minBill, Config.maxBill)
				...
				if Player.Functions.AddItem(itemData["name"], amount, false, info) then
					TriggerClientEvent('xt-notify:client:Alert', source,"THÔNG BÁO", "Bạn đã gửi cho " ..GetPlayerName(tonumber(args[1])).." "..amount.." "..itemData["name"].. "", 5000, 'success')
				else
					TriggerClientEvent('xt-notify:client:Alert', source,"THÔNG BÁO", 'Không thể gửi vật phẩm', 5000, 'error')
					
				end
			else
				TriggerClientEvent('xt-notify:client:Alert', source,"THÔNG BÁO", 'Vật phẩm không tồn tại', 5000, 'error')
				
			end
		else
			TriggerClientEvent('xt-notify:client:Alert', source,"THÔNG BÁO", 'Số lượng không hợp lệ', 5000, 'error')
		end
	else
		TriggerClientEvent('xt-notify:client:Alert', source,"THÔNG BÁO", 'Người chơi không online', 5000, 'error')
	end
end, "admin")

QBCore.Commands.Add("randomitems", "Trao vật phẩm ngẫu nhiên (God Only)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	local filteredItems = {}
	for k, v in pairs(QBCore.Shared.Items) do
		if QBCore.Shared.Items[k]["type"] ~= "weapon" then
			table.insert(filteredItems, v)
		elseif QBCore.Shared.Items[k]["type"] ~= "tool" then
			table.insert(filteredItems, v)
		end
	end
	for i = 1, 10, 1 do
	...
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if dist < 3.0 then
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>Họ:</strong> {1} <br><strong>Tên:</strong> {2} <br><strong>Ngày sinh:</strong> {3} <br><strong>Bằng mã:</strong> {4}</div></div>',
					args = {
						"Bằng Lái",
						item.info.firstname,
						item.info.lastname,
...
QBCore.Functions.CreateUseableItem("id_card", function(source, item)
	...
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if dist < 3.0 then
			...
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>Mã công dân:</strong> {1} <br><strong>Họ:</strong> {2} <br><strong>Tên:</strong> {3} <br><strong>Ngày sinh:</strong> {4} <br><strong>Giới tính:</strong> {5} <br><strong>Quốc tịch:</strong> {6}</div></div>',
					args = {
						"Căn Cước Công Dân",
						item.info.citizenid,
						item.info.firstname,
