...
Citizen.CreateThread(function()
    ...
    if Config.Framework == "esx" then
        frameworkObject.RegisterServerCallback("xt-hud:GetMoney", function(source, cb, moneytype)
            local src = source
...
RegisterServerEvent('xt-hud:RemoveItem')
AddEventHandler('xt-hud:RemoveItem', function(item, amount)
    local src = source
...
Citizen.CreateThread(function()
    ...
    for _,v in pairs(GetPlayers()) do
        ...
        if player ~= nil then
            ...
            TriggerClientEvent('xt-hud:client:UpdateSettings', v,  preferences[identifier])
            TriggerClientEvent('xt-hud:SetForceHide', v, false)
            TriggerClientEvent('xt-hud:UpdateNitroData', v, nitro)
            TriggerClientEvent('xt-hud:Loaded', v)
        end
    end
end)
...
function CheckPreferencesExist(identifier)
    if preferences[identifier] == nil then
        ...
        if data.hideBoxData == nil then
            ...
            if data.hideBoxData.pee == nil then
                data.hideBoxData.pee = 100
                updated = true

            end
            if data.hideBoxData.poo == nil then
                data.hideBoxData.poo = 100
                updated = true
...
if Config.EnableCashAndBankCommands then
    RegisterCommand(Config.CashCommand, function(source)
        if Config.Framework == "esx" then
            ...
            TriggerClientEvent('xt-hud:ShowAccounts', source, 'cash', cashamount)
        else
    ...
    RegisterCommand(Config.BankCommand, function(source)
        if Config.Framework == "esx" then
            ...
            if bankamount then
                TriggerClientEvent('xt-hud:ShowAccounts', source, 'bank', bankamount.money)
            end
        else
            local Player = frameworkObject.Functions.GetPlayer(tonumber(source))
            local bankamount = Player.PlayerData.money.bank
            TriggerClientEvent('xt-hud:ShowAccounts', source, 'bank', bankamount)
...
RegisterNetEvent('xt-hud:UpdateData')
AddEventHandler("xt-hud:UpdateData", function(settingstype, val)
    local src = source
...
RegisterServerEvent('xt-hud:server:EjectPlayer')
AddEventHandler('xt-hud:server:EjectPlayer', function(table, velocity)
   for i=1, #table do
        if table[i] then
            TriggerClientEvent("xt-hud:client:EjectPlayer", table[i], velocity)
        end
    end
end)