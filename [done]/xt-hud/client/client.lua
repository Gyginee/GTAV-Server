...
RegisterNetEvent('xt-hud:Loaded')
AddEventHandler('xt-hud:Loaded', function()
    playerLoaded = true
end)
...
RegisterNetEvent('xt-hud:OnAddedMoney')
AddEventHandler("xt-hud:OnAddedMoney", function(amount, minus, moneyType)
...
Citizen.CreateThread(function()
    ...
    SendNUIMessage({ type="set_EnablePee",  value = Config.EnablePee})
    SendNUIMessage({ type="set_EnablePoo",  value = Config.EnablePoo})
...
RegisterNetEvent('xt-hud:SetForceHide')
AddEventHandler('xt-hud:SetForceHide', function(val)
    while true do
        Citizen.Wait(0)
...
RegisterNetEvent('xt-hud:ShowAccounts')
AddEventHandler('xt-hud:ShowAccounts', function(type, money)
...
RegisterNetEvent('xt-hud:client:UpdateSettings')
AddEventHandler('xt-hud:client:UpdateSettings', function(data)
...
function CinematicShow(bool)
    ...
    TriggerEvent('xt-hud:client:UpdateSettings', preferencesData)
    if bool then
...
if Config.EnableSeatbelt then
    ...
    RegisterNetEvent('xt-hud:seatbelt:toggle')
    AddEventHandler('xt-hud:seatbelt:toggle', function(toggle)
    ...
    RegisterNetEvent('xt-hud:client:EjectPlayer')
    AddEventHandler('xt-hud:client:EjectPlayer', function(velocity)
	    ...
	    while true do
	        ...
	        if car ~= 0 and (wasInCar or IsCar(car)) then
                ...
                if speedBuffer[2] and GetEntitySpeedVector(car, true).y > 1.0  and speedBuffer[1] > 15 and (speedBuffer[2] - speedBuffer[1]) > (speedBuffer[1] * 0.255) then			   
                    ...
                    if #seatPlayerId > 0 then TriggerServerEvent("xt-hud:server:EjectPlayer", seatPlayerId, velBuffer[2]) end                    
	        	end
...
if Config.EnableEngineToggle then
    ...
    RegisterNetEvent('xt-hud:client:ToggleEngine')
    AddEventHandler('xt-hud:client:ToggleEngine', function(toggle)
        local vehicle = GetVehiclePedIsIn(playerPed, false)
        if vehicle == 0 or GetPedInVehicleSeat(vehicle, -1) ~= playerPed then return end
        if not toggle then
            Config.Notification(Config.Notifications["engine_off"]["message"], Config.Notifications["engine_off"]["type"])
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPauseMenuActive() and not pauseActive then
            pauseActive = true
            TriggerEvent('xt-hud:SetForceHide', true)
        end
        if not IsPauseMenuActive() and pauseActive and not CinematicModeOn then
            pauseActive = false
            TriggerEvent('xt-hud:SetForceHide', false)
        end
    end
end)
