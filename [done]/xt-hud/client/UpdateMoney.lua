        ...
        while true do
            if frameworkObject ~= nil then
                if frameworkObject.GetPlayerData() then
                    ...
                    frameworkObject.TriggerServerCallback("xt-hud:GetMoney", function(cash)
                        SendNUIMessage({
                            type = "update_money",
                            money = cash
                        })
...
AddEventHandler("es:addedMoney", function(a, b, m)
    ...
    if Config.EnableWatermarkCash then
        TriggerEvent('xt-hud:OnAddedMoney', m - lastCash, false, "cash")
        lastCash = m
    end
end)
...
AddEventHandler("es:removedMoney", function(a, b, m)
    ...
    if Config.EnableWatermarkCash then
        TriggerEvent('xt-hud:OnAddedMoney', (m - lastCash) * -1, true, "cash")
        lastCash = m
    end
end)
    ...
    if Config.EnableWatermarkCash then
        if lastCash ~= nil and lastCash ~= data.money.cash then
            if  data.money.cash > lastCash then
                TriggerEvent('xt-hud:OnAddedMoney', data.money.cash - lastCash, false , "cash")
            else
                TriggerEvent('xt-hud:OnAddedMoney', (data.money.cash - lastCash) * -1, true , "cash")
            end
    ...
    if Config.EnableWatermarkBankMoney then
        if lastBank ~= nil  and lastBank ~= data.money.bank then
            if data.money.bank > lastBank then
                TriggerEvent('xt-hud:OnAddedMoney', data.money.bank - lastBank, false, "bank")
            else
                TriggerEvent('xt-hud:OnAddedMoney', (data.money.bank - lastBank)* -1, true, "bank")
            end
        ...
        if Config.EnableWatermarkBankMoney then
           if lastBank ~= nil  and lastBank ~= money then
                if money > lastBank then
                    TriggerEvent('xt-hud:OnAddedMoney',money - lastBank, false, "bank")
                else
                    TriggerEvent('xt-hud:OnAddedMoney', (money - lastBank) * -1, true, "bank")
                end
                lastBank = money
            end     
        end
        ...
        if Config.EnableWatermarkCash then
            if lastCash ~= nil  and lastCash ~= money then
                if money > lastCash then
                    ...
                    TriggerEvent('xt-hud:OnAddedMoney',money - lastCash, false, "cash")
                else
                    TriggerEvent('xt-hud:OnAddedMoney',  (money - lastCash ) * -1, true, "cash")
                end
                lastCash = money
            end
        end
