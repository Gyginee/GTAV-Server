...
Config.EnablePee = true
Config.EnablePoo = true
...
Config.VehicleEngineToggleKey = "H" -- Default control key for toggle engine. Players can change the key according to their desire 
...
Config.SettingsLocale = { -- Settings texts
    ...
    ["enable_cinematicmode"] = "Chế độ Cinematic",
    ["disable_cinematicmode"] = "Tắt Cinematic",
    ["exit_settings_1"] = "Thoát",
    ["exit_settings_2"] = "Thoát",
...
Config.Notification = function(message, type, isServer, src) -- You can change here events for notifications
    if isServer then
        if Config.Framework == "esx" then
            ...
            TriggerClientEvent('xt-notify:Alert', src, message, type, 1500)
        end
    else
        if Config.Framework == "esx" then
            exports["okokNotify"]:Alert("HUD", message, 3000, type)
        else
            TriggerEvent('xt-notify:Alert', message, type, 1500)
        end
    end
end 