...
const app = Vue.createApp({
  data: function () {
    return {
      ...
      isDynamicPooChecked: this.initIsDynamicPeeChecked(),
      isDynamicPooChecked: this.initIsDynamicPooChecked(),
  ...
  watch: {
    ...
    isDynamicPeeChecked: function () {
      localStorage.setItem("isDynamicPeeChecked", this.isDynamicPeeChecked);
    },
    isDynamicPooChecked: function () {
      localStorage.setItem("isDynamicPooChecked", this.isDynamicPooChecked);
    },
  ...
  methods: {
    ...
    initIsDynamicPeeChecked: function () {
      const stored = localStorage.getItem("isDynamicPeeChecked");
      if (stored === null) {
        return false;
      } else {
        return stored == 'true';
    ...
    initIsDynamicPooChecked: function () {
      const stored = localStorage.getItem("isDynamicPooChecked");
      if (stored === null) {
        return false;
      } else {
        return stored == 'true';
    ...
    dynamicPee: function (event) {
      targetId = event.currentTarget.id;
      dynamicPee()
    },
    dynamicPoo: function (event) {
      targetId = event.currentTarget.id;
      dynamicPoo()
    },
...
function dynamicPee() {
  $.post('https://qb-hud/dynamicPee');
}
function dynamicPoo() {
  $.post('https://qb-hud/dynamicPoo');
}
...
const playerHud = {
  ...
  methods: {
    hudTick(data) {
      ...
      } else if (data.dynamicPee == false) {
        this.showPee = true;
      }
      if (data.pee >= 90) {
        this.peeColor = "#edf500";
      } else if (data.pee <= 20) {
        this.peeColor = "#ff0000";
      } else {
        this.peeColor = "#f3fa28";
      }

      if (data.dynamicPoo == true) {
        if (data.poo == 100) {
          this.showPoo = false;
        }
        else {
      ...
      } else if (data.dynamicPoo == false) {
        this.showPoo = true;
      }
      if (data.poo >= 90) {
        this.pooColor = "#b84102";
      } else if (data.poo <= 20) {
        this.pooColor = "#ff0000";
      } else {
        this.pooColor = "#bf5c28";
      }
