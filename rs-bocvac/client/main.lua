...
Citizen.CreateThread(function ()
    ...
    SetBlipSprite(RecycleBlip, 445)
    SetBlipColour(RecycleBlip, 34)
    ...
    while true do
        ...
        if #(pos - vector3(Config['delivery'].outsideLocation.x, Config['delivery'].outsideLocation.y, Config['delivery'].outsideLocation.z)) < 1.3 then
            DrawText3D(Config['delivery'].outsideLocation.x, Config['delivery'].outsideLocation.y, Config['delivery'].outsideLocation.z + 1, "~g~E~w~ - để vào kho hàng")
        ...
        ---- BINNEN
            ...
            if #(pos - vector3(Config['delivery'].insideLocation.x, Config['delivery'].insideLocation.y, Config['delivery'].insideLocation.z)) < 1.5 then
                if not onDuty then
                    DrawText3D(Config['delivery'].insideLocation.x, Config['delivery'].insideLocation.y, Config['delivery'].insideLocation.z + 1, "~g~E~w~ - để rời khỏi")
                    if IsControlJustReleased(0, 38) then
                        TriggerServerEvent('qb-bocvac:server:traluong')
                        DoScreenFadeOut(500)
                        while not IsScreenFadedOut() do
                            Citizen.Wait(10)
                        end
                    ...
                    DrawText3D(Config['delivery'].insideLocation.x, Config['delivery'].insideLocation.y, Config['delivery'].insideLocation.z + 1, " ~r~Giao hàng và kết thúc thì mới ra được")
                end
            end
        ...
        if #(pos - vector3(1049.15, -3100.63, -39.95)) < 15 and not IsPedInAnyVehicle(PlayerPedId(), false) and carryPackage == nil then
            ...
            if #(pos - vector3(1049.15, -3100.63, -39.95)) < 1.3 then
                if onDuty then
                    DrawText3D(1049.15, -3100.63, -38.95, "~g~E~w~ - ~r~Kết thúc")                
                else
                    DrawText3D(1049.15, -3100.63, -38.95, "~g~E~w~ -  ~g~Bắt đầu")
                end
                if IsControlJustReleased(0, 38) then
                    DropPackage()
                    onDuty = not onDuty
                    if onDuty then
                        QBCore.Functions.Notify("Bạn đã bắt đầu", "success")
                    else
                        QBCore.Functions.Notify("Bạn đã dừng lại", "error")
                    end
                end
            end
        end
            ...
            if packagePos ~= nil then
                ...
                if carryPackage == nil then
                    if #(pos - vector3(packagePos.x, packagePos.y, packagePos.z)) < 2.3 then
                        DrawText3D(packagePos.x,packagePos.y,packagePos.z+ 1, "~g~E~w~ - Đóng gói")
                        if IsControlJustReleased(0, 38) then
                            DropPackage()
                            ScrapAnim()
                            QBCore.Functions.Progressbar("pickup_reycle_package", "Đóng gói nhận hàng ..", math.random(4000, 6000), false, true, {
                        ...
                        DrawText3D(packagePos.x, packagePos.y, packagePos.z + 1, "Gói hàng")
                    end
                else
                    if #(pos - vector3(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z)) < 2.0 then
                        DrawText3D(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z, "~g~E~w~ - Giao hàng")
                        if IsControlJustReleased(0, 38) then
                            DropPackage()
                            ScrapAnim()
                            QBCore.Functions.Progressbar("deliver_reycle_package", "Giao gói hàng", 5000, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                            }, {}, {}, {}, function() -- Done
                                -- local tile = math.random(1, 100)  -- KHO BÁU
                                -- if tile <= 1 then
                                    -- TriggerServerEvent('khobau:nhanduocbando2')
                                -- end
                                -- local tile2 = math.random(1, 100)
                                -- if tile2 <= 1 then
                                    -- TriggerServerEvent('khobau:nhanduocbando4')
                                -- end
                                StopAnimTask(PlayerPedId(), "mp_car_bomb", "car_bomb_mechanic", 1.0)
                                TriggerServerEvent('qb-bocvac:server:getItem')
                                GetRandomPackage()
                            end)
                        end
                    else
                        DrawText3D(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z, "Giao hàng")
                    end
                end
            else
                GetRandomPackage()
