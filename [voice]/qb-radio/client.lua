...
local function connecttoradio(channel)
    ...
    if SplitStr(tostring(channel), ".")[2] ~= nil and SplitStr(tostring(channel), ".")[2] ~= "" then
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã kết nối đến tần số <span style='color:#069a19'><b>"..channel.. " MHz</b></span>!", 5000, 'success')
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã kết nối đến tần số <span style='color:#069a19'><b>"..channel.. ".00 MHz</b></span>!", 5000, 'success')
    end
end
...
local function leaveradio()
    ...
    exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã rời khỏi tần số này", 5000, 'error')
end
            ...
            if rchannel ~= RadioChannel then
                if Config.RestrictedChannels[rchannel] ~= nil then
                    if Config.RestrictedChannels[rchannel][PlayerData.job.name] and PlayerData.job.onduty then
                        ...
                        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không thể kết nối tần số này", 5000, 'error')
                    end
                else
                    connecttoradio(rchannel)
                end
            else
                exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã kết nối tần số này", 5000, 'error')
            end
        else
            exports['xt-notify']:Alert("THÔNG BÁO", "Tần số không xác định", 5000, 'error')
        end
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Tần số không xác định", 5000, 'error')
    end
...
RegisterNUICallback('leaveRadio', function(data, cb)
    if RadioChannel == 0 then
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn chưa trong tần số nào!", 5000, 'error')
    else
        leaveradio()
    end
end)
...
RegisterNUICallback("volumeUp", function()
	if RadioVolume <= 95 then
        ...
        exports['xt-notify']:Alert("THÔNG BÁO", "Volume mới là <span style='color:#069a19'><b>"..RadioVolume.."</b></span>", 5000, 'success')
		exports["pma-voice"]:setRadioVolume(RadioVolume)
	else
        exports['xt-notify']:Alert("THÔNG BÁO", "Đây đã là volume lớn nhất", 5000, 'error')
	end
end)
...
RegisterNUICallback("volumeDown", function()
	if RadioVolume >= 10 then
		...
		exports['xt-notify']:Alert("THÔNG BÁO", "Đây đã là volume nhỏ nhất", 5000, 'error')
	end
end)
...
RegisterNUICallback("increaseradiochannel", function(data, cb)
    ...
    exports['xt-notify']:Alert("THÔNG BÁO", "Tần số mới là <span style='color:#069a19'><b>"..newChannel.."</b></span>", 5000, 'success')
end)
