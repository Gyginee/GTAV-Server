    ...
    if whitelistedVehicle() then
       -- if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                exports['okokNotify']:Alert("Hệ thống", "Vị trí của người dân đã định vị trên GPS", 5000, 'info')
                    ...
                    while not NpcData.NpcTaken do
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                DrawText3D(Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].x, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].y, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].z, '[E] Đón khách')
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    exports['okokNotify']:Alert("Hệ thống", "Đưa người dân đến vị trí mong muốn", 5000, 'info')
                ...
                exports['okokNotify']:Alert("Hệ thống", "Bạn đang thực hiện nhiệm vụ đưa đón NPC", 5000, 'warning')
      ...
      --  else
       ...
       -- end
        ...
        exports['okokNotify']:Alert("Hệ thống", "Bạn không trong xe Taxi", 5000, 'error')
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    local npccoords = GetEntityCoords(NpcData.Npc)
                    DrawText3D(Config.NPCLocations.DeliverLocations[NpcData.CurrentDeliver].x, Config.NPCLocations.DeliverLocations[NpcData.CurrentDeliver].y, Config.NPCLocations.DeliverLocations[NpcData.CurrentDeliver].z, '[E] Trả khách')
                    if IsControlJustPressed(0, 38) then
                        local veh = GetVehiclePedIsIn(ped, 0)
                        ...
                        exports['okokNotify']:Alert("Hệ thống", "Khách đã xuống xe", 5000, 'success')
            ...
            if isLoggedIn then
                ...
                if PlayerData.job.name == "taxi" then
                    ...
                    if vehDist < 30 then
                        ...
                        if vehDist < 1.5 then
                            if whitelistedVehicle() then
                                DrawText3D(Config.Location.x, Config.Location.y, Config.Location.z + 0.3, '[E] Bãi đậu xe')
                                ...
                                DrawText3D(Config.Location.x, Config.Location.y, Config.Location.z + 0.3, '[E] Phương tiện công việc')
                                if IsControlJustReleased(0, 38) then
                                    TaxiGarage()
...
AddEventHandler('qb-taxi:client:toggleMeter', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then 
            ...
            exports['okokNotify']:Alert("Hệ thống", "Phương tiện không có đồng hồ đo", 5000, 'error')
            --QBCore.Functions.Notify('This Vehicle Has No Taxi Meter', 'error')
        end
    else
        exports['okokNotify']:Alert("Hệ thống", "Bạn không trong phương tiện", 5000, 'error')
...
AddEventHandler('qb-taxi:client:enableMeter', function()
    ...
    if meterIsOpen then
        ...
        exports['okokNotify']:Alert("Hệ thống", "Đồng hồ không hoạt động", 5000, 'error')
...
AddEventHandler('qb-taxi:client:toggleMuis', function()
    ...
    if meterIsOpen then
        ...
        exports['okokNotify']:Alert("Hệ thống", "Không có đồng hồ taxi trong tầm nhìn", 5000, 'error')
...
function TakeVehicle(k)
    ...
    QBCore.Functions.SpawnVehicle(Config.AllowedVehicles[k].model, function(veh)
        ...
        exports['qg-vehiclekeys']:SetVehicleKey(GetVehicleNumberPlateText(veh), true)
...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("TaxiCorp")
