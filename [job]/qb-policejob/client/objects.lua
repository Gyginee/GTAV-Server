...
RegisterNetEvent('police:client:spawnCone', function()
    QBCore.Functions.Progressbar("spawn_object", "Đặt vật thể", 2500, false, true, {
...
RegisterNetEvent('police:client:deleteObject', function()
    ...
    if dist < 5.0 then
        QBCore.Functions.Progressbar("remove_object", "Xoá vật thể..", 2500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
...
RegisterNetEvent('police:client:SpawnSpikeStrip', function()
    if #SpawnedSpikes + 1 < Config.MaxSpikes then
        ...
        exports['xt-notify']:Alert("THÔNG BÁO", "Không thể đặt đinh nhọn !", 5000, 'error')
    end
end)
            ...
            if ClosestSpike then
                ...
                if dist < 4 then
                    if not IsPedInAnyVehicle(PlayerPedId()) then
                        if PlayerJob.name == "police" and PlayerJob.onduty then
                            sleep = 0
                            DrawText3D(pos.x, pos.y, pos.z, "[~r~E~s~] Xoá đinh nhọn")
