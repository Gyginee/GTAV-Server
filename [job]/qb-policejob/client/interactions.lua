...
RegisterNetEvent('police:client:SearchPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        exports['xt-notify']:Alert("THÔNG BÁO", "Không có ai ở gần bạn", 5000, 'error')
    end
end)
...
RegisterNetEvent('police:client:Capbangvukhi', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("police:server:Capbangvukhi", playerId)
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Không có ai ở gần bạn", 5000, 'error')
    end
end)
RegisterNetEvent('police:client:thubangvukhi', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("police:server:thubangvukhi", playerId)
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Không có ai ở gần bạn", 5000, 'error')
    end
end)
...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) then
            QBCore.Functions.Progressbar("robbing_player", "Sờ nắn", math.random(5000, 7000), false, true, {
                ...
                StopAnimTask(ped, "random@shop_robbery", "robbery_action_b", 1.0)
                exports['xt-notify']:Alert("THÔNG BÁO", "Hủy", 5000, 'error')
            end)
        end
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Không có ai ở gần bạn", 5000, 'error')
...
RegisterNetEvent('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(dialog['jailtime']) > 0 then
            ...
            exports['xt-notify']:Alert("THÔNG BÁO", "Số tháng phải nhiều hơn 0", 5000, 'error')
        end
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Không có ai ở gần bạn", 5000, 'error')
    end
    ...
    if not IsPedRagdoll(PlayerPedId()) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                ...
                exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không thể còng người ngồi trong xe", 5000, 'error')
            end
        else
            exports['xt-notify']:Alert("THÔNG BÁO", "Không có ai ở gần bạn", 5000, 'error')
        end
        ...
        if player ~= -1 and distance < 1.5 then
            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                if result then
                    ...
                    exports['xt-notify']:Alert("THÔNG BÁO", "Không có "..QBCore.Shared.Items[Config.HandCuffItem].label, 5000, 'error')
                end
...
RegisterNetEvent('police:client:GetUnCuffed', function(playerId)
    if isHandcuffed ~= true then
        ...
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "Uncuff", 0.2)
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã được thả", 5000, 'error')
    end
end)
...
RegisterNetEvent('police:client:GetCuffed', function(playerId, isSoftcuff)
    local ped = PlayerPedId()
    local time = math.random(6,9) -- Time on how fast the circle goes around. The lower the value = faster it is!
    if not isHandcuffed then
        ...
        if success then
            ...
            exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã thoát khỏi còng tay", 5000, 'success')
        else
            Wait(0)
            isHandcuffed = true
            ...
            if not isSoftcuff then
                cuffType = 16
                GetCuffedAnimation(playerId)
                exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã bị còng", 5000, 'error')
            else
                cuffType = 49
                GetCuffedAnimation(playerId)
                exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã bị còng tay nhưng chân thì không", 5000, 'error')
            end
        end
    else
        isHandcuffed = false
        ...
        ClearPedTasksImmediately(ped)
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "Uncuff", 0.2)
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã được thả còng", 5000, 'success')
    end
end)
