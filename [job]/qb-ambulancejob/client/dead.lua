...
function OnDeath()
    if not isDead then
        isDead = true
        TriggerServerEvent("xt-benhvien:server:SetDeathStatus", true)
        ...
        if isDead then
            ...
            TriggerServerEvent('xt-benhvien:server:ambulanceAlert', "Có người dân bị chết")
        end
    end
end
    ...
    while isDead do
        ...
        if deathTime <= 0 then
            if IsControlPressed(0, 38) and hold <= 0 and not isInHospitalBed then
                TriggerEvent("xt-benhvien:client:RespawnAtHospital")
                hold = 5
            end
	...
	while true do
		...
		if NetworkIsPlayerActive(player) then
            ...
            elseif IsEntityDead(playerPed) and InLaststand and not isDead then
                ...
                local killerName = killerId ~= -1 and GetPlayerName(killerId) .. " " .. "("..GetPlayerServerId(killerId)..")" or "Tự tử hoặc do NPC giết"
		...
		if isDead or InLaststand then
            ...
            if isDead then
                if not isInHospitalBed then
                    if deathTime > 0 then
                        local conlai = math.ceil(deathTime)
                        DrawTxt(0.85, 1.4, 1.0, 1.0, 0.45, "HỒI SINH SAU: ~r~"..conlai.."~s~ GIÂY", 255, 255, 255, 255)
                    else
                        DrawTxt(0.85, 1.4, 1.0, 1.0, 0.45, "GIỮ [~r~E~s~] TRONG "..hold.." GIÂY ĐỂ HỒI SINH VỀ BỆNH VIỆN VỚI GIÁ $~r~"..Config.BillCost.."~s~", 255, 255, 255, 255)
                    end
                end
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Laststand.MinimumRevive then
                    DrawTxt(0.85, 1.4, 1.0, 1.0, 0.45,"BẠN SẼ CHẾT SAU: ~r~"..time.."~s~ GIÂY" , 255, 255, 255, 255)
                else
                    DrawTxt(0.85, 1.4, 1.0, 1.0, 0.45,"BẠN SẼ CHẾT SAU: ~r~"..time.."~s~ GIÂY, BẠN CẦN ĐƯỢC GIÚP ĐỠ" , 255, 255, 255, 255)
                    if not emsNotified then
                        DrawTxt(1.91, 0.40, 1.0, 1.0, 0.6, "NHẤN [~r~G~s~] ĐỂ ĐƯỢC GIÚP ĐỠ'", 255, 255, 255, 255)
                    else
                        DrawTxt(1.90, 1.40, 1.0, 1.0, 0.6, "NHÂN VIÊN Y TẾ SẼ ĐẾN SỚM THÔI", 255, 255, 255, 255)
                    end

                    if IsControlJustPressed(0, 47) and not emsNotified then
                        TriggerServerEvent('xt-benhvien:server:ambulanceAlert', "Người dân bị thương")
                        emsNotified = true
                    end
                end
