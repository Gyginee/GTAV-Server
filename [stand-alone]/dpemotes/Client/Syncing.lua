        ...
        if #args > 0 then
            ...
            if(distance ~= -1 and distance < 3) then
                if DP.Shared[emotename] ~= nil then
                    ...
                    exports['xt-notify']:Alert("THÔNG BÁO", "Gửi lời mời đến <span style='color:#0092ff'>"..GetPlayerName(target).." </span>("..ename..")", 5000, 'info')
                else
                    EmoteChatMessage("'"..emotename.."' "..Config.Languages[lang]['notvalidsharedemote'].."")
                end
            else
                exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không gần ai cả", 5000, 'info')
            end
        else
          MearbysOnCommand()
        end
