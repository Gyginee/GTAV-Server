...
AddEventHandler('animations:client:EmoteCommandStart', function(args)
  if CanDoEmote then
    ...
    exports['xt-notify']:Alert("THÔNG BÁO", "Không thể hoàn thành ngay bây giờ", 5000, 'error')
  end
end)
...
function EmoteChatMessage(args)
  if args == display then
    TriggerEvent("chatMessage", "^5Giúp đỡ^0", {0,0,0}, string.format(""))
  else
    TriggerEvent("chatMessage", "^5Giúp đỡ^0", {0,0,0}, string.format(""..args..""))
  end
end
...
function EmoteCommandStart(source, args, raw)
    ...
    if name == "c" then
        if IsInAnimation then
          ...
          exports['xt-notify']:Alert("THÔNG BÁO", "Không thể huỷ", 5000, 'error')
        end
      return
    ...
    elseif DP.PropEmotes[name] ~= nil then
      ...
      exports['xt-notify']:Alert("THÔNG BÁO", "Lệnh không khả dụng", 5000, 'error')
    end
  end
end
