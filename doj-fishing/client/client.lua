	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Config.StopFishing) then
				...
				exports['okokNotify']:Alert("THẤT BẠI", "Bạn đã dừng câu cá", 3000, 'error')
			end
			...
			if fishing then
				...
				if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z -3.0)  then
					...
					exports['okokNotify']:Alert("THẤT BẠI", "Mực nước không đủ sâu để bắt đầu câu", 3000, 'error')
				end
				if IsEntityDead(playerPed) or IsEntityInWater(playerPed) then
					endFishing()
					exports['okokNotify']:Alert("THẤT BẠI", "Kết thúc câu cá", 3000, 'error')
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					exports['okokNotify']:Alert("THẤT BẠI", "Cá đã cắn đứt cước câu!! Bạn đen lắm.", 3000, 'error')
...
CreateThread(function()
	while true do
		...
		if fishing then
			pause = true
			correct = 1
			TriggerEvent('3dme:triggerDisplay', 'Cần câu bắt đầu kéo!')
			exports['textUi']:DrawTextUi('show', "Nhấn [F] để bắt cá!")
			input = 0
			pausetimer = 0
		end
	end
...
RegisterNetEvent('fishing:client:progressBar', function()
	QBCore.Functions.Notify('Mở hộp', 'primary', 1000)
end)
...
RegisterNetEvent('fishing:client:attemptTreasureChest', function()
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
		if HasItem then
			QBCore.Functions.Progressbar("accepted_key", "Đưa chìa khóa..", (math.random(2000, 5000)), false, true, {
		  ...
		  QBCore.Functions.Notify("Bạn không có chìa khóa", "error")
		end
	  end, 'fishingkey')
end)
...
RegisterNetEvent('fishing:SkillBar', function(message)
	...
	if Config.Skillbar == "reload-skillbar" then
		...
		if finished ~= 100 then
			QBCore.Functions.Notify('Cá đã lọt lưới rồi!', 'error')
			loseBait()
		else
			local finished2 = exports["reload-skillbar"]:taskBar(math.random(2500,5000),math.random(3,5))
			if finished2 ~= 100 then
				QBCore.Functions.Notify('Đứt cước cần câu rồi, thu lưới thôi', 'error')
				loseBait()
			else
	...
	elseif Config.Skillbar == "onemore-skillbar" then
		local Skillbar = exports['onemore-skillbar']:GetSkillbarObject()
		Skillbar.Start({
			...
			QBCore.Functions.Notify('Cá đã trốn thoát', 'error')
			loseBait()
		end)
	end
end) 
...
-- 	TriggerServerEvent("inventory:server:OpenInventory", "stash", 'FishingBox_'..BoxId, {maxweight = 18000, slots = 25})
...
RegisterNetEvent('fishing:fishstart', function()
	...
	local pos = GetEntityCoords(playerPed) 
	if IsPedSwimming(playerPed) then return QBCore.Functions.Notify("Bạn không thể bơi và câu cá cùng một lúc.", "error") end 
	if IsPedInAnyVehicle(playerPed) then return QBCore.Functions.Notify("Bạn cần ra khỏi ghế ngồi trên thuyền để câu.", "error") end 
	if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z - 3.0)  then
		local time = 1000
		QBCore.Functions.Notify('Sử dụng cần câu', 'primary', time)
		Wait(time)
		exports['textUi']:DrawTextUi('show', "Nhấn [X] để dừng câu")
		fishAnimation()
	else
		exports['okokNotify']:Alert("THẤT BẠI","Bạn cần phải đi xa bờ hơn", 1500, 'error')
...
--[[RegisterNetEvent('thuethuyen')
AddEventHandler('thuethuyen',function(data)
    local ped = PlayerPedId()
	...
	if data.location == 1 then 
        if Config.Fish24hours then
            ...
            if GetClockHours() >= 6 and GetClockHours() <= 21 then
                ...
                QBCore.Functions.Notify('Đã quá trễ! Thời gian mở cửa từ 6:00am tới 21:00pm', 'error')
            end
        end
	elseif data.location == 2 then
        if Config.Fish24hours then
            ...
            if GetClockHours() >= 6 and GetClockHours() <= 21 then
                ...
                QBCore.Functions.Notify('Quá trễ! Thời gian mở cửa từ 6:00am tới 21:00pm', 'error')
            end
        end
            ...
            if DoesEntityExist(currVeh) then
                if IsThisModelABoat(vehModel) or IsThisModelAJetski(vehModel) or IsThisModelAnAmphibiousCar(vehModel) or IsThisModelAnAmphibiousQuadbike(vehModel) then
                    if IsBoatAnchoredAndFrozen(currVeh) then
						QBCore.Functions.Notify('Lấy neo', 'success')
                        Wait(2000)
						QBCore.Functions.Notify('Neo đã được kéo lên', 'primary')
                    ...
                    elseif not IsBoatAnchoredAndFrozen(currVeh) and CanAnchorBoatHere(currVeh) and GetEntitySpeed(currVeh) < 3 then
                        ...
                        QBCore.Functions.Notify('Thả neo', 'primary')
                        Wait(2000)
						QBCore.Functions.Notify('Thả neo', 'success')
...
loseBaitAnimation = function()
	...
	exports['textUi']:DrawTextUi('show', "Cá đã cắn mồi!")
	Wait(2000)
	exports['textUi']:DrawTextUi('hide')
	fishAnimation()
end
...
catchAnimation = function()
	...
	TaskPlayAnim(ped, animDict, animName, 1.0, -1.0, 1.0, 0, 0, 0, 48, 0)
	local time = 1750
	QBCore.Functions.Notify('Câu dính cá!', 'success', time)
	Wait(time)
	TriggerServerEvent('fishing:server:catch') 
	loseBait()
	if math.random(1, 100) < 50 then
		TriggerServerEvent('onemore-hud:Server:RelieveStress', 50)
	end
...
fishAnimation = function()
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
		if HasItem then
		  ...
		  endFishing()
		  QBCore.Functions.Notify("Bạn không còn mồi, hãy đi mua thêm đi", "error")
		end
	end, 'fishbait')
end
...
attemptTreasureChest = function()
	...
	RemoveAnimDict(animDict)
	QBCore.Functions.Notify('Cố gắng mở Rương kho báu', 'primary', 1500)
	Wait(1500)
...
openedTreasureChest = function()
	if math.random(1,15) == 10 then
		...
		QBCore.Functions.Notify("Chìa khóa đã bị gãy", "error", 7500)
	end
	...
	QBCore.Functions.Notify("Rương kho báu đã mở! Hãy chắc chắn để thu thập tất cả chiến lợi phẩm của bạn!!", "success", 7500)
	local ShopItems = {} 
