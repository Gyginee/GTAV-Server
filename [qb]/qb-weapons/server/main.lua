    ...
    if Inventory[ItemData.slot] then
        if Inventory[ItemData.slot].info.attachments and next(Inventory[ItemData.slot].info.attachments) then
            ...
            if HasAttach then
                ...
                TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Bạn đã <span style='color:#ff1100'>tháo </span> "..QBCore.Shared.Items[AttachmentComponent.item].label.."!", 5000, 'error')
    ...
    if Player.PlayerData.items[data.slot] then
        if Player.PlayerData.items[data.slot].info.quality then
            if Player.PlayerData.items[data.slot].info.quality ~= 100 then
                if Player.Functions.RemoveMoney('cash', Config.WeaponRepairCosts[WeaponClass]) then
                    ...
                    SetTimeout(Timeout, function()
                        ...
                        TriggerEvent('qb-phone:server:sendNewMailToOffline', Player.PlayerData.citizenid, {
                            sender = "Gyn bụng bia",
                            subject = "Sửa chữa",
                            message = WeaponData.label.." của bạn đã được sửa chữa xong. Bạn có thể đến cửa hàng lấy nó.<br><br>Chúc bạn một ngày tốt lành"
                        })
                ...
                TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Vũ khí này không bị <span style='color:#ff1100'>hỏng hóc</span>!", 5000, 'error')            
                cb(false)
            end
        else
            TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Vũ khí này không bị <span style='color:#ff1100'>hỏng hóc</span>!", 5000, 'error')
        ...
        TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Bạn không có <span style='color:#ff1100'>vũ khí</span> trên tay!", 5000, 'error')
            ...
            if WeaponSlot.info.quality then
                for i = 1, RepeatAmount, 1 do
                    if WeaponSlot.info.quality - DecreaseAmount > 0 then
                        ...
                        TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Vũ khí của bạn đã <span style='color:#ff1100'>hỏng hóc</span>. Hãy sửa nó!", 5000, 'error')
                        break
                    end
                end
            else
    ...
    if Inventory[CurrentWeaponData.slot] then
        if Inventory[CurrentWeaponData.slot].info.attachments and next(Inventory[CurrentWeaponData.slot].info.attachments) then
            ...
            if not HasAttach then
                ...
                TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "<span style='color:#ff1100'>"..QBCore.Shared.Items[AttachmentData.item].label.."</span> đã được lắp trên vũ khí!", 5000, 'error')
            end
        else
...
QBCore.Commands.Add("repairweapon", "Sửa vũ khí (Chỉ GOD)", {{name="hp", "Độ bền của vũ khí"}}, true, function(source, args)
