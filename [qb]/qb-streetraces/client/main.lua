        ...
        if Races ~= nil then
                ...
                for k, race in pairs(Races) do
                    if Races[k] ~= nil then
                        if #(pos - vector3(Races[k].startx, Races[k].starty, Races[k].startz)) < 15.0 and not Races[k].started then
                            DrawText3Ds(Races[k].startx, Races[k].starty, Races[k].startz, "[~g~H~w~] Để tham gia cuộc đúa (~g~$"..Races[k].amount..",-~w~)")
            ...
            if RaceId ~= 0 and InRace then
                if #(pos - vector3(Races[RaceId].endx, Races[RaceId].endy, pos.z)) < 250.0 and Races[RaceId].started then
                    DrawText3Ds(Races[RaceId].endx, Races[RaceId].endy, pos.z + 0.98, "HOÀN THÀNH")
            ...
            if ShowCountDown then
                if #(pos - vector3(Races[RaceId].startx, Races[RaceId].starty, Races[RaceId].startz)) < 15.0 and Races[RaceId].started then
                    DrawText3Ds(Races[RaceId].startx, Races[RaceId].starty, Races[RaceId].startz, "Cuộc đua sẽ bắt đầu sau ~g~"..RaceCount)
                end
            end
        end
    end
...
RegisterNetEvent('qb-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        exports['xt-notify']:Alert("THÔNG BÁO", "Cuộc đua kết thúc! Người thắng cuộc là: "..winner, 5000, 'error')
    end
end)
...
RegisterNetEvent('qb-streetraces:CreateRace', function(amount)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if #(pos - vector3(c[1], c[2], groundZ)) > 500.0 then
            ...
            exports['xt-notify']:Alert("THÔNG BÁO", "Tiền cược là: "..amount, 5000, 'success')
        else
            exports['xt-notify']:Alert("THÔNG BÁO", "Điểm đích quá gần", 5000, 'error')
        end
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn cần đặt điểm đích", 5000, 'error')
    end
end)
...
function RaceCountDown()
    ...
    while RaceCount ~= 0 do
        ...
        exports['xt-notify']:Alert("THÔNG BÁO", RaceCount, 2000, 'info')
        Wait(1000)
    ...
    exports['xt-notify']:Alert("THÔNG BÁO", "Đạp gaaaaaaa!!!", 2000, 'info')
end
