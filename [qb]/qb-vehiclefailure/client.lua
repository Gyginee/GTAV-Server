...
local function CleanVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("cleaning_vehicle", "Rửa phương tiện", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function() -- Done
		exports['xt-notify']:Alert("THÔNG BÁO", "Phương tiện đã được làm sạch", 5000, 'success')
...
local function RepairVehicleFull(vehicle)
	...
	QBCore.Functions.Progressbar("repair_vehicle", "Sửa phương tiện", math.random(20000, 30000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		exports['xt-notify']:Alert("THÔNG BÁO", "Phương tiện đã được sửa chữa", 5000, 'success')
...
local function RepairVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("repair_vehicle", "Sửa phương tiện", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
		exports['xt-notify']:Alert("THÔNG BÁO", "Phương tiện đã được làm sửa", 5000, 'success')
...
RegisterNetEvent('qb-vehiclefailure:client:RepairVehicle', function()
	...
	if vehicle ~= nil and vehicle ~= 0 and engineHealth < 500 then
		...
		if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
			...
			if #(pos - drawpos) < 2.0 and not IsPedInAnyVehicle(ped) then
      		...
      		if #(pos - vehpos) > 4.9 then
       			exports['xt-notify']:Alert("THÔNG BÁO", "Không có phương tiện nào ở gần bạn", 5000, 'error')
			else
       			exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không thể ở trong phương tiện", 5000, 'error')
      		end
		end
  	else
		if vehicle == nil or vehicle == 0 then
			...
			exports['xt-notify']:Alert("THÔNG BÁO", "Xe của bạn đã bị hỏng quá nặng", 5000, 'error')
		end
	end
end)
...
RegisterNetEvent('iens:repaira', function()
	if isPedDrivingAVehicle() then
		...
		SetVehicleUndriveable(vehicle, false)
		WashDecalsFromVehicle(vehicle, 1.0)
		exports['xt-notify']:Alert("THÔNG BÁO", "Phương tiện đã được sữa chửa", 5000, 'success')
		...
		exports['xt-notify']:Alert("THÔNG BÁO", "Bạn phải ở trong phương tiện để sửa chữa", 5000, 'error')
	end
end)

RegisterNetEvent('iens:besked', function()
	exports['xt-notify']:Alert("THÔNG BÁO", "Có hỗ trợ bên đường có sẵn cuộc gọi thông qua điện thoại của bạn!", 5000, 'error')
end)

RegisterNetEvent('iens:notAllowed', function()
	exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không đủ quyền hạn", 5000, 'error')
end)
	...
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				exports['xt-notify']:Alert("THÔNG BÁO", "Phương tiện của bạn đã ổn", 5000, 'success')
			else
				exports['xt-notify']:Alert("THÔNG BÁO", "Phương tiện bị hỏng quá nặng", 5000, 'error')
			end
		else
			exports['xt-notify']:Alert("THÔNG BÁO", "Phương tiện bị hỏng quá nặng", 5000, 'error')
		end
