...
local function CreateInstuctionScaleform(scaleform)
    ...
    InstructionButtonMessage("Thoát Camera")
    PopScaleformMovieFunctionVoid()
...
function HouseKeysMenu()
    ...
    if holders == nil or next(holders) == nil then
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không có <span style='color:#ff1100'>chìa khoá</span> của căn nhà!", 5000, 'error')
        CloseMenuFull()
    else
        keyholderMenu = {}
...
local function getDataForHouseTier(house, coords)
    ...
    if not shells[houseTier] then
        exports['xt-notify']:Alert("THÔNG BÁO", "Không tìm thấy <span style='color:#ff1100'>khung</span> căn nhà!", 5000, 'error')
        return nil
    else
...
RegisterNetEvent('qb-houses:client:addGarage', function()
    if ClosestHouse ~= nil then
        ...
        exports['xt-notify']:Alert("THÔNG BÁO", "Không có <span style='color:#ff1100'>căn nhà</span> nào ở gần bạn!", 5000, 'error')
    end
end)
    ...
    if dist <= 1.5 then
        if HasHouseKey then
            if Config.Houses[ClosestHouse].locked then
                ...
                exports['xt-notify']:Alert("THÔNG BÁO", "Đã <span style='color:#4fff00'>mở khoá</span> căn nhà!", 3000, 'success')
            else
                TriggerServerEvent('qb-houses:server:lockHouse', true, ClosestHouse)
                exports['xt-notify']:Alert("THÔNG BÁO", "Đã <span style='color:#ff1100'>khoá</span> căn nhà!", 3000, 'error')
            end
        else
            exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không có <span style='color:#ff1100'>chìa khoá</span> của căn nhà này!", 5000, 'error')
        end
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Không có <span style='color:#ff1100'>căn nhà</span> nào ở gần bạn!", 5000, 'error')
    end
...
RegisterNetEvent('qb-houses:client:RingDoor', function(player, house)
    if ClosestHouse == house and IsInside then
        ...
        exports['xt-notify']:Alert("THÔNG BÁO", "Có ai đó đang <span style='color:#008df7'>ngoài cửa</span>!", 5000, 'info')
    end
end)
...
RegisterNetEvent('qb-houses:client:giveHouseKey', function()
    ...
    if player ~= -1 and distance < 2.5 and ClosestHouse ~= nil then
        ...
        if housedist < 10 then
            ...
            exports['xt-notify']:Alert("THÔNG BÁO", "Không có <span style='color:#ff1100'>cái cửa</span> nào ở gần bạn!", 5000, 'error')
        end
    elseif ClosestHouse == nil then
        exports['xt-notify']:Alert("THÔNG BÁO", "Không có <span style='color:#ff1100'>căn nhà</span> nào ở gần bạn!", 5000, 'error')
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Không có <span style='color:#ff1100'>ai</span> nào ở gần bạn!", 5000, 'error')
    end
end)
        ...
        if housedist <= 5 then
            QBCore.Functions.TriggerCallback('qb-houses:server:getHouseOwner', function(result)
                if QBCore.Functions.GetPlayerData().citizenid == result then
                    ...
                    exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không sở hữu <span style='color:#ff1100'>căn nhà</span> này!", 5000, 'error')
                end
            end, ClosestHouse)
        else
            exports['xt-notify']:Alert("THÔNG BÁO", "Không có <span style='color:#ff1100'>cái cửa</span> nào ở gần bạn!", 5000, 'error')
...
RegisterNetEvent('qb-houses:client:setupHouseBlips2', function() -- Setup unowned on load
    for k,v in pairs(Config.Houses) do
        if not v.owned then
            ...
            AddTextEntry('UnownedHouse', "Nhà đang bán")
...
RegisterNetEvent('qb-houses:client:createBlip', function(coords) -- Create unowned on command
    ...
    AddTextEntry('NewHouseBlip', "Nhà đang bán")
...
RegisterNetEvent('qb-houses:client:setLocation', function(data)
    ...
    if IsInside then
        ...
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không <span style='color:#ff1100'>ở trong</span> căn nhà này!", 5000, 'error')
    end
end)
                ...
                if dist < 1 then
                    if Config.Houses[ClosestHouse].locked then
                        if not Config.Houses[ClosestHouse].IsRaming then
                            ...
                            exports['xt-notify']:Alert("THÔNG BÁO", "Có ai đó đang <span style='color:#ff1100'>phá cửa</span> của căn nhà này!", 5000, 'error')
                        end
                    else
                        exports['xt-notify']:Alert("THÔNG BÁO", "Căn nhà đã được <span style='color:#ff1100'>mở khoá</span>", 5000, 'error')
                    end
                else
                    exports['xt-notify']:Alert("THÔNG BÁO", "Không có <span style='color:#ff1100'>căn nhà</span> nào ở gần bạn!", 5000, 'error')
                end
            else
                exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không đủ <span style='color:#ff1100'>quyền hạn</span>!", 5000, 'error')
            end
        end)
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Không có <span style='color:#ff1100'>căn nhà</span> nào ở gần bạn!", 5000, 'error')
...
RegisterNetEvent('qb-houses:client:ResetHouse', function()
    if ClosestHouse ~= nil then
        ...
        if Config.Houses[ClosestHouse].IsRammed then
            ...
            TriggerServerEvent('qb-houses:server:lockHouse', true, ClosestHouse)
            RamsDone = 0
            exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã phá được <span style='color:#4fff00'>khoá</span> căn nhà!", 3000, 'success')
        else
            exports['xt-notify']:Alert("THÔNG BÁO", "Căn nhà chưa được <span style='color:#ff1100'>phá khoá</span>!", 5000, 'error')
        end
    end
end)
                ...
                if IsInside and CurrentHouse ~= nil and not entering and isOwned then
                    if stashLocation ~= nil then
                        ...
                        elseif #(pos - vector3(stashLocation.x, stashLocation.y, stashLocation.z)) <= 3 then
                            DrawText3Ds(stashLocation.x, stashLocation.y, stashLocation.z, "Tủ đồ")
                        end
                    end
                    ...
                    if outfitLocation ~= nil then
                        ...
                        elseif #(pos - vector3(outfitLocation.x, outfitLocation.y, outfitLocation.z)) <= 3 then
                            DrawText3Ds(outfitLocation.x, outfitLocation.y, outfitLocation.z, "Tủ quần áo")
                        end
                    end
                    ...
                    if logoutLocation ~= nil then
                        ...
                        elseif #(pos - vector3(logoutLocation.x, logoutLocation.y, logoutLocation.z)) < 3 then
                            DrawText3Ds(logoutLocation.x, logoutLocation.y, logoutLocation.z, "Nhân vật")
                        end
                    end
                end
