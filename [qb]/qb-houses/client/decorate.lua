...
local function CheckMovementInput()
	...
	if IsControlJustReleased(0, 21) then -- Left Shift
		...
		exports['xt-notify']:Alert("THÔNG BÁO", "Tốc độ là <span style='color:#008df7'>"..tostring(speeds[curSpeed]).."</span>!", 5000, 'info')
	end
...
RegisterNetEvent('qb-houses:client:decorate', function()
	...
	if IsInside then
		...
		exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không <span style='color:#ff1100'>ở trong</span> căn nhà!", 5000, 'error')
	end
end)
	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã <span style='color:#ff1100'>rời khỏi</span> căn nhà!", 5000, 'error')
			end
		end
	end
end)
