...
QBCore.Commands.Add("decorate", "Trang trí nội thất", {}, false, function(source)
    local src = source
    TriggerClientEvent("qb-houses:client:decorate", src)
end)

QBCore.Commands.Add("createhouse", "Tạo bất động sản", {{name = "price", help = "Giá căn nhà"}, {name = "tier", help = "Khung nhà"}}, true, function(source, args)
    local src = source
    ...
    if Player.PlayerData.job.name == "realestate" then
        ...
        TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Bạn không phải <span style='color:#ff1100'>nhân viên Bất động sản</span>!", 5000, 'error')
    end
end)

QBCore.Commands.Add("addgarage", "Thêm gara cho căn nhà", {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "realestate" then
...
QBCore.Commands.Add("ring", "Bấm chuông", {}, false, function(source)
    local src = source
...
QBCore.Functions.CreateUseableItem("police_stormram", function(source, item)
    ...
    if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
        ...
        TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Bạn phải là <span style='color:#ff1100'>cảnh sát</span> trong ca làm!", 5000, 'error')
    end
end)
...
RegisterNetEvent('qb-houses:server:addNewHouse', function(street, coords, price, tier)
    ...
    local ten = pData.PlayerData.charinfo.firstname.." "..pData.PlayerData.charinfo.lastname
    ...
    TriggerClientEvent("qb-houses:client:setHouseConfig", -1, Config.Houses)
    TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Bạn đã tạo căn nhà tại địa chỉ <span style='color:#008df7'>"..label.."</span>!", 5000, 'info')
    TriggerEvent('qb-log:server:CreateLog', 'house', "House Created", 'green', "**Địa chỉ**: "..label.."\n\n**Giá**: "..price.."\n\n**Khung nhà **: "..tier.."\n\n**Người tạo**: "..ten.."("..GetPlayerName(src)..")")
end)
...
RegisterNetEvent('qb-houses:server:buyHouse', function(house)
    ...
    if isOwned then
        TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Căn nhà này đã có <span style='color:#ff1100'>chủ</span>!", 5000, 'error')
        CancelEvent()
        return
    end
    ...
    if (bankBalance >= HousePrice) then
        ...
        local ten = pData.PlayerData.charinfo.firstname.." "..pData.PlayerData.charinfo.lastname
        TriggerEvent('qb-log:server:CreateLog', 'house', "Mua nhà", 'green', "**Địa chỉ**: "..house:upper().."\n\n**Giá mua**: "..HousePrice.."\n\n**Người mua**: "..ten.."("..GetPlayerName(src)..")")
    else
        TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Bạn không đủ <span style='color:#ff1100'>tiền</span>!", 5000, 'error')
    end
end)
...
RegisterNetEvent('qb-houses:server:removeHouseKey', function(house, citizenData)
    ...
    local ten = citizenData.firstname.." "..citizenData.lastname
    TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Đã xoá <span style='color:#ff1100'>chìa khoá từ</span> "..ten.."!", 5000, 'error')
...
RegisterNetEvent('qb-houses:server:giveHouseKey', function(target, house)
    ...
    if tPlayer then
        if housekeyholders[house] then
            for _, cid in pairs(housekeyholders[house]) do
                if cid == tPlayer.PlayerData.citizenid then
                    TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Người này đã có <span style='color:#ff1100'>chìa khoá</span>", 5000, 'error')
                    return
                end
            end
            ...
            TriggerClientEvent('xt-notify:client:Alert', tPlayer.PlayerData.source,"THÔNG BÁO", "Bạn đã nhận <span style='color:#4fff00'>chìa khoá</span> căn nhà ở <span style='color:#4fff00'>"..Config.Houses[house].adress.."</span>", 5000, 'success')
        else
        ...
        TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Vui lòng thử lại", 5000, 'error')
    end
end)
