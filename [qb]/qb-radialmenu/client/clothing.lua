...
function ToggleClothing(whic, extra)
	...
	if which ~= "Mask" then
		if not Gender then exports['xt-notify']:Alert("THÔNG BÁO", "Nhân vật không đáp ứng được <span style='color:#fc1100'>tuỳ chọn</span> này", 5000, 'error') return false end -- We cancel the command here if the person is not using a multiplayer model.
	end
	...
	if not Toggle.Table.Standalone then -- "Standalone" is for things that dont require a variant, like the shoes just need to be switched to a specific drawable. Looking back at this i should have planned ahead, but it all works so, meh!
		...
		exports['xt-notify']:Alert("THÔNG BÁO", "Không có <span style='color:#fc1100'>tuỳ chọn</span> nào đáp ứng", 5000, 'error') return
	else
		if not LastEquipped[which] then
	...
	exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã <span style='color:#ffd700'>mặc nó</span>", 5000, 'warning') return false
end
...
function ToggleProps(whic)
	...
	if not Prop.Variants then
		...
		exports['xt-notify']:Alert("THÔNG BÁO", "Không có gì để <span style='color:#fc1100'>cởi</span>!", 5000, 'error') return false
	else
		local Gender = IsMpPed(Ped)
		if not Gender then exports['xt-notify']:Alert("THÔNG BÁO", "Nhân vật không đáp ứng được <span style='color:#fc1100'>tuỳ chọn</span> này", 5000, 'error') return false end -- We dont really allow for variants on ped models, Its possible, but im pretty sure 95% of ped models dont really have variants.
		...
		exports['xt-notify']:Alert("THÔNG BÁO", "Không có <span style='color:#fc1100'>tuỳ chọn</span> nào đáp ứng", 5000, 'error') return false
	end
end
