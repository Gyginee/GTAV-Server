...
RegisterNetEvent('qb-trunk:client:KidnapTrunk', function()
    ...
    if distance ~= -1 and distance < 2 then
        if isKidnapping then
            ...
            exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không thể<span style='color:#fc1100'>bắt cóc</span> người này", 5000, 'error')
        end
    end
end)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã chui vào trong <span style='color:#30ff00'><b>cốp xe</b></span>!", 5000, 'success')
                                TrunkCam(true)
                                isKidnapped = true
                            else
                                exports['xt-notify']:Alert("THÔNG BÁO", "Cốp xe này đã bị <span style='color:#fc1100'>khoá</span>!", 5000, 'error')
                            end
                        else
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                ClearPedTasks(ped)
                                inTrunk = false
                                TriggerServerEvent('qb-trunk:server:setTrunkBusy', plate, nil)
                        ...
                        exports['xt-notify']:Alert("THÔNG BÁO", "Có ai đó đang ở trong cốp xe", 5000, 'error')
                    end
                else
                    exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã ở trong cốp xe", 5000, 'error')
                end
            else           
                exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không thể chui vào cốp xe này", 5000, 'error')
            end
        end, plate)
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không thể chui vào cốp xe này", 5000, 'error')
...
RegisterNetEvent('qb-trunk:client:GetIn', function()
    ...
    if closestVehicle ~= 0 then
        ...
        exports['xt-notify']:Alert("THÔNG BÁO", "Không tìm thấy phương tiện", 5000, 'error')
    end
end)
        ...
        if inTrunk then
            if not isKidnapped then
                ...
                if DoesEntityExist(vehicle) then
                    sleep = 0
                    DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.75, "Nhấn [~g~E~s~] - Ra khỏi cốp")
                    ...
                    if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                        sleep = 0
                        DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.5, "Nhấn [~g~G~s~] - Đóng cốp")
                        ...
                        sleep = 0
                        DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.5, "Nhấn [~g~G~s~] - Mở cốp")
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
