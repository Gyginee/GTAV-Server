...
RegisterNetEvent('qb-radialmenu:client:TakeStretcher', function()
    ...
    if vehicle ~= 0 then
        ...
        if obj ~= 0 then
            ...
            exports['xt-notify']:Alert("THÔNG BÁO", "Không thể đặt <span style='color:#fc1100'>vật phẩm </span>này", 5000, 'error')
        end
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không gần một <span style='color:#fc1100'>nhân viên y tế</span> nào", 5000, 'error')
    end
end)
...
RegisterNetEvent('qb-radialmenu:client:RemoveStretcher', function()
    ...
    if stretcherObject then
        ...
        if #(coords - bCoords) < 3.0 then
            ...
            exports['xt-notify']:Alert("THÔNG BÁO", "Bạn ở <span style='color:#fc1100'>quá xa</span> nào", 5000, 'error')
        end
    end
end)
...
RegisterNetEvent('qb-radialmenu:client:Result', function(isBusy, type)
    ...
    if type == "lay" then
        if not isBusy then
            ...
            exports['xt-notify']:Alert("THÔNG BÁO", "Cáng đã có <span style='color:#fc1100'>người</span>", 5000, 'error')
            isLayingOnBed = false
        end
    else
        if not isBusy then
        ...
        if stretcherObject then
            ...
            if distance <= 1.0 then
                if not isAttached then
                    sleep = 0
                    DrawText3Ds(offsetCoords.x, offsetCoords.y, offsetCoords.z, "Nhấn [~g~E~s~] - Đẩy cáng")
                    ...
                    sleep = 0
                    DrawText3Ds(offsetCoords.x, offsetCoords.y, offsetCoords.z, "Nhấn [~g~E~s~] - Ngừng đẩy")
                    if IsControlJustPressed(0, 51) then
                        detachStretcher()
                        isAttached = false
                        sleep = 100
                ...
                if not isLayingOnBed then
                    if not isAttached then
                        sleep = 0
                        DrawText3Ds(offsetCoords.x, offsetCoords.y, offsetCoords.z + 0.2, "Nhấn [~g~G~s~] - Đặt vào cáng")
            ...
            elseif distance <= 2 then
                if not isLayingOnBed then
                    sleep = 0
                    DrawText3Ds(offsetCoords.x, offsetCoords.y, offsetCoords.z, "Tại đây")
                else
                    if not isAttached then
                        sleep = 0
                        DrawText3Ds(offsetCoords.x, offsetCoords.y, offsetCoords.z + 0.2, "Nhấn [~g~G~s~] - Xuống cáng")
                        if IsControlJustPressed(0, 47) or IsDisabledControlJustPressed(0, 47) then
                            getOffStretcher()
                            sleep = 100
                        end
