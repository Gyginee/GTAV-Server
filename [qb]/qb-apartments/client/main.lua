...
function MenuOwners()
    QBCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        if next(apartments) == nil then
            exports['xt-notify']:Alert("THÔNG BÁO", "Ở đây không có <span style='color:#ff0000'>căn hộ</span> nào cả!", 5000, 'error')
            closeMenuFull()
        else
            local vehicleMenu = {
                {
...
RegisterNetEvent('apartments:client:SpawnInApartment', function(apartmentId, apartment)
    ...
    if rangDoorbell ~= nil then
        ...
        if doorbelldist > 5 then
            exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã đi quá xa <span style='color:#ff0000'>căn hộ</span>!", 5000, 'error')
            return
        end
    end
...
RegisterNetEvent('apartments:client:RingDoor', function(player, house)
    ...
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "doorbell", 0.1)
    exports['xt-notify']:Alert("THÔNG BÁO", "Có ai đó đang <span style='color:#008df7'>bấm chuông</span>!", 5000, 'info')
end)
