    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            ...
            if (data.toggle) {
                ...
                var originalText = "Lấy giá trị nhân vật";
                ...
                var DotsInterval = setInterval(function() {
                    ...
                    if (loadingProgress == 3) {
                        originalText = "Xác thực giá trị nhân vật"
                        $("#loading-text").html(originalText);
                    }
                    if (loadingProgress == 4) {
                        originalText = "Truy vấn nhân vật"
                        $("#loading-text").html(originalText);
                    }
                    ...
                    setTimeout(function(){
                        ...
                        $(".title-screen").fadeIn(0, function() {
                            setTimeout(function() {
                                ...
                                $('.jugadores-on').html(' ' + data.players + ' Người chơi');
                            }, 1000);
...
function setupCharInfo(cData) {
    if (cData == 'empty') {
        $('.character-info-valid').html('<span id="no-char">Chọn nhân vật để xem thông tin.</span>');
    } else {
        var gender = "Nam"
        ...
        '<div class="character-info-box"><span id="info-label">Tiền: </span><span class="char-info-js">&#36; '+cData.money.cash+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Tài khoản: </span><span class="char-info-js">&#36; '+cData.money.bank+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Số điện thoại: </span><span class="char-info-js">'+cData.charinfo.phone+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Số tài khoản: </span><span class="char-info-js">'+cData.charinfo.account+'</span></div>');
    }
}
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $("#play-text").html('<i class="fa fa-sign-in" aria-hidden="true"></i> Chơi');
            $("#delete-text").html('<i class="fa fa-trash" aria-hidden="true"></i> Xoá');
            $("#play").css({"display":"block"});
        ...
        if ((selectedChar).data('cid') == "") {
            $(selectedChar).addClass("char-selected");
            setupCharInfo('empty')
            $("#play-text").html('<i class="fa fa-plus" aria-hidden="true"></i> Đăng ký');
            $("#play").css({"display":"block"});
            $("#delete").css({"display":"none"});
            $.post('https://qb-multicharacter/cDataPed', JSON.stringify({
                cData: cDataPed
...
function setCharactersList() {
    var htmlResult = '<div class="character-list-header"><p>Nhân vật</p></div>'
    for (let i = 1; i <= NChar; i++) {
        htmlResult += '<div class="character" id="char-'+ i +'" data-cid=""><span id="slot-name">Trống<span id="cid"></span></span></div>'
    }
    htmlResult += '<div class="character-btn" id="play"><p id="play-text">Chọn nhân vật</p></div><div class="character-btn" id="delete"><p id="delete-text">Chọn nhân vật</p></div>'
