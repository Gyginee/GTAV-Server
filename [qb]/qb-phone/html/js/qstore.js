...
SetupAppstore = function(data) {
    ...
    $.each(data.StoreApps, function(i, app){
        if (data.PhoneData.InstalledApps[i] == null || data.PhoneData.InstalledApps[i] == undefined) {
            ...
            var elem = '<div class="storeapp" id="app-'+i+'" data-app="'+i+'"><div class="storeapp-icon"><i class="'+app.icon+'"></i></div><div class="storeapp-title">'+app.title+'<span style="font-size: 1vh;"> - Cài đặt</span></div> <div class="storeapp-creator">'+app.creator+'</div><div class="storeapp-remove"><i class="fas fa-trash"></i></div></div>'
            $(".store-apps").append(elem);
            app.app = i;
            $("#app-"+i).data('AppData', app);
        }
