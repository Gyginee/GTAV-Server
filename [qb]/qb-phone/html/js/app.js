...
$(document).on('click', '.phone-application', function(e){
    ...
    if (AppObject.length !== 0) {
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $("#mySerialNumber").text("XT-" + QB.Phone.Data.PlayerData.metadata["phonedata"].SerialNumber);
                    ...
                    $.post('https://qb-phone/GetPlayerKeys', JSON.stringify({}), function(Keys){
                        ...
                        if (Keys.length > 0) {
                            $.each(Keys, function(i, key){
                                var elem = '<div class="mykeys-key" id="keyid-'+i+'"><span class="mykeys-key-label">' + key.HouseData.adress + '</span> <span class="mykeys-key-sub">Ấn để lấy GPS</span> </div>';
        ...
        if (PressedApplication != null){
            QB.Phone.Notifications.Add("fas fa-exclamation-circle", "HỆ THỐNG", QB.Phone.Data.Applications[PressedApplication].tooltipText+" không khả dụng!")
        }
    }
});
        ...
        switch(event.data.action) {
            ...
            case "UpdateCallTime":
                ...
                if (!QB.Phone.Data.IsOpen) {
                    ...
                    $(".call-notifications-title").html("Trong cuộc gọi ("+timeString+")");
                    $(".call-notifications-content").html("Với "+event.data.Name);
                ...
                $(".phone-currentcall-title").html("Trong cuộc gọi ("+timeString+")");
                break;
