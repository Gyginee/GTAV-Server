...
$(document).on('click', '.racing-race', function(e){
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "ĐUA", "Cuộc đua đã bắt đầu..", "#1DA1F2");
    }
});
...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            var Locked = '<i class="fas fa-unlock"></i> Chưa bắt đầu';
            if (race.RaceData.Started) {
                Locked = '<i class="fas fa-lock"></i> Đã bắt đầu';
            }
            var LapLabel = "";
            if (race.Laps == 0) {
                LapLabel = "NƯỚC RÚT"
            ...
            var Buttons = '<div class="race-buttons"> <div class="race-button" id="join-race" data-toggle="racetooltip" data-placement="left" title="Tham gia"><i class="fas fa-sign-in-alt"></i></div>';
            if (InRace) {
                if (!Creator) {
                    Buttons = '<div class="race-buttons"> <div class="race-button" id="quit-race" data-toggle="racetooltip" data-placement="right" title="Thoát"><i class="fas fa-sign-out-alt"></i></div>';
                } else {
                    if (!race.RaceData.Started) {
                        Buttons = '<div class="race-buttons"> <div class="race-button" id="start-race" data-toggle="racetooltip" data-placement="left" title="Bắt đầu"><i class="fas fa-flag-checkered"></i></div><div class="race-button" id="quit-race" data-toggle="racetooltip" data-placement="right" title="Quit"><i class="fas fa-sign-out-alt"></i></div>';
                    } else {
                        Buttons = '<div class="race-buttons"> <div class="race-button" id="quit-race" data-toggle="racetooltip" data-placement="right" title="Thoát"><i class="fas fa-sign-out-alt"></i></div>';
                    }
                }
                ...
                if (InDistance) {
                    $.post('https://qb-phone/IsBusyCheck', JSON.stringify({
                        ...
                        if (!IsBusy) {
                            ...
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "ĐUA", "Bạn đang chỉnh sửa..", "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "ĐUA", "Bạn đã trong cuộc đua..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('https://qb-phone/GetTrackData', JSON.stringify({
        ...
        $(".racing-setup-information-distance").html('Khoảng cách: '+TrackData.Distance+' m');
        $(".racing-setup-information-creator").html('Người tạo: ' + CreatorTag);
            ...
            if (!data.IsBusy) {
                $.post('https://qb-phone/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "ĐUA", "Bạn trong đường đua..", "#1DA1F2");
                    }
                });
            } else {
                QB.Phone.Notifications.Add("fas fa-flag-checkered", "ĐUA", "Bạn đã thiết lập một đường đua..", "#1DA1F2");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "ĐUA", "Bạn không có quyền tạo Đường đua..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        ...
        if (TrackName == '') TrackName = 'Bạn đang cố làm gì vậy?'
        $.post('https://qb-phone/IsAuthorizedToCreateRaces', JSON.stringify({
            ...
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-flag-checkered", "ĐUA", "Tên không khả dụng..", "#1DA1F2");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-flag-checkered", "ĐUA", "Bạn không có quyền tạo Đường đua..", "#1DA1F2");
            }
        ...
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "ĐUA", "Bạn không nhập tên đường đua..", "#1DA1F2");
    }
});
                            ...
                            $.post('https://qb-phone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    ...
                                    $(".racing-setup").animate({
                                        ...
                                        $(".racing-setup-information-distance").html('Chọn đường đua');
                                        $(".racing-setup-information-creator").html('Chọn đường đua');
                                        $(".racing-setup-information-wr").html('Chọn đường đua');
                                        $(".racing-setup-laps").val("");
                                        $('.dropdown').find('input').removeAttr('value');
                                        $('.dropdown').find('span').text("Chọn đường đua");
                                    });
                                } else {
                                    QB.Phone.Notifications.Add("fas fa-flag-checkered", "ĐUA", "Không thể ..", "#1DA1F2");
                                }
                            });
                        } else {
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "ĐUA", "Điền vào một số lượng vòng..", "#1DA1F2");
                        }
                    } else {
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "ĐUA", "Bạn chưa chọn đường đua..", "#1DA1F2");
                    }
                }
            })
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "ĐUA", "Bạn đã có một cuộc đua đang hoạt động..", "#1DA1F2");
        }
    });
});
        ...
        if (Races !== null) {
            ...
            $.each(Races, function(i, race){
                if (race.LastLeaderboard.length > 0) {
                    var elem = '<div class="racing-leaderboard-item" id="leaderboard-item-'+i+'"> <span class="racing-leaderboard-item-name"><i class="fas fa-flag-checkered"></i> '+race.RaceName+'</span> <span class="racing-leaderboard-item-info">Bấm để biết thêm chi tiết</span> </div>'
