...
QB.Phone.Functions.GetLastMessage = function(messages) {
    ...
    $.each(messages[messages.length - 1], function(i, msg){
        ...
        if(LastMessageData.message == '') 'hmm con mèo kêu meo meo...'
    });
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        ...
        QB.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "Bạn không thể gửi một tin nhắn trống rỗng!", "#25D366", 1750);
    }
});
    ...
    if (OpenedChatData.number !== null) {
        if(e.which === 13){
            ...
            if (Message !== null && Message !== undefined && Message !== "") {
                ...
                if (clean == '') clean = 'Con lợn kêu ủn ỉn...'
                $.post('https://qb-phone/SendMessage', JSON.stringify({
                    ChatNumber: OpenedChatData.number,
                    ChatDate: GetCurrentDateKey(),
                    ChatMessage: clean,
    ...
    if (cData) {
        ...
        $.each(cData.messages, function(i, chat){
            ...
            $.each(cData.messages[i].messages, function(index, message){
                ...
                if (message.message == '') message.message = 'Hmm, bạn không nên làm như thế...'
                var Sender = "me";
        ...
        var ChatDiv = '<div class="whatsapp-openedchat-messages-'+DateString+' unique-chat"><div class="whatsapp-openedchat-date">HÔM NAY</div></div>';
