...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(QB.Phone.Data.PlayerData.charinfo.firstname+"."+QB.Phone.Data.PlayerData.charinfo.lastname+"@xteam.com");
    $("#mail-header-lastsync").html("Lần đồng bộ cuối "+MessageTime);
    if (Mails !== null && Mails !== undefined) {
        if (Mails.length > 0) {
            ...
            $(".mail-list").html('<p class="nomails">Bạn không có thư nào..</p>');
        }
...
QB.Phone.Functions.RefreshAdverts = function(Adverts) {
    ...
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $.each(Adverts, function(i, advert){
            ...
            if (clean == '') { clean = 'Tôi là một con slime vô hại :/' }
...
$(document).on('click','#adv-delete',function(e){
    ...
    $.post('https://qb-phone/DeleteAdvert', function(){
        setTimeout(function(){
            QB.Phone.Notifications.Add("fas fa-ad", "Quảng cáo", "Quảng cáo đã được gỡ bỏ", "#ff8f1a", 2000);
        },400)
    });
})
