...
$(document).on('click', '.person-search-result', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Tên: '+ClickedPersonData.firstname+' '+ClickedPersonData.lastname+'</div> <div class="person-search-result-bsn">CSN: '+ClickedPersonData.citizenid+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Ngày sinh: '+ClickedPersonData.birthdate+'</div> <div class="person-search-result-number">Số điện thoại: '+ClickedPersonData.phone+'</div> <div class="person-search-result-nationality">Quốc tịch: '+ClickedPersonData.nationality+'</div> <div class="person-search-result-gender">Giới tính: '+Gender+'</div> &nbsp; <div class="person-search-result-apartment"><span id="'+ClickedPersonId+'">Căn hộ: '+appartementData.label+'</span> <i class="fas fa-map-marker-alt appartment-adress-location" id="'+ClickedPersonId+'"></i></div> &nbsp; <div class="person-search-result-warned">Tiền án: '+IsWarrant+'</div> <div class="person-search-result-driverslicense">Bằng lái: '+HasLicense+'</div>';
    ...
    if (OpenedPerson === null) {
        ...
        var PreviousElement = '<div class="person-search-result-name">Tên: '+PreviousPersonData.firstname+' '+PreviousPersonData.lastname+'</div> <div class="person-search-result-bsn">CSN: '+PreviousPersonData.citizenid+'</div>';
...
$(document).on('click', '.person-search-result-apartment > span', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "MDT", "Sao chép số nhà!", "#badc58", 1750);
...
$(document).on('click', '.person-search-result-house', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Chủ sở hữu: '+ClickedHouseData.charinfo.firstname+' '+ClickedHouseData.charinfo.lastname+'</div><div class="person-search-result-bsn">Nhà: '+ClickedHouseData.label+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Address: '+ClickedHouseData.label+' &nbsp; <i class="fas fa-map-marker-alt house-adress-location" id="'+ClickedHouseId+'"></i></div> <div class="person-search-result-number">Cấp: '+ClickedHouseData.tier+'</div> <div class="person-search-result-nationality">Garage: ' + GarageLabel + '</div>';
    ...
    if (OpenedHouse === null) {
        ...
        var PreviousElement = '<div class="person-search-result-name">Chủ sở hữu: '+PreviousPersonData.charinfo.firstname+' '+PreviousPersonData.charinfo.lastname+'</div> <div class="person-search-result-bsn">Nhà: '+PreviousPersonData.label+'</div>';
        ...
        $.post('https://qb-phone/FetchSearchResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, person) {
                    var PersonElement = '<div class="person-search-result" id="person-'+i+'"><div class="person-search-result-name">Tên: '+person.firstname+' '+person.lastname+'</div> <div class="person-search-result-bsn">CSN: '+person.citizenid+'</div> </div>';
                    $(".person-search-results").append(PersonElement);
                    $("#person-"+i).data("PersonData", person);
                });
            } else {
                QB.Phone.Notifications.Add("politie", "MDT", "Không có kết quả tìm kiếm!");
                $(".person-search-results").html("");
            }
        });
    } else {
        ...
        $.post('https://qb-phone/FetchPlayerHouses', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, house) {
                    var PersonElement = '<div class="person-search-result-house" id="personhouse-'+i+'"><div class="person-search-result-name">Chủ sở hữu: '+house.charinfo.firstname+' '+house.charinfo.lastname+'</div> <div class="person-search-result-bsn">House: '+house.label+'</div></div>';
                    $(".person-search-results").append(PersonElement);
...
$(document).on('click', '.scan-search-vehicle', function(e){
    ...
    $.post('https://qb-phone/FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            ...
            var VehicleElement = '<div class="vehicle-search-result"> <div class="vehicle-search-result-name">'+vehicle.label+'</div> <div class="vehicle-search-result-plate">Biển số: '+vehicle.plate+'</div> <div class="vehicle-opensplit"></div> &nbsp; <div class="vehicle-search-result-owner">Chủ sở hữu: '+vehicle.owner+'</div> &nbsp; <div class="vehicle-search-result-apk">MOT: '+APK+'</div> <div class="vehicle-search-result-warrant">Tiền án: '+Flagged+'</div> </div>'
            $(".vehicle-search-results").append(VehicleElement);
        } else {
            QB.Phone.Notifications.Add("politie", "MDT", "Không có phương tiện nào gần đây!");
...
AddPoliceAlert = function(data) {
    ...
    if (data.alert.coords != undefined && data.alert.coords != null) {
        AlertElement = '<div class="meos-alert" id="alert-'+randId+'"> <span class="meos-alert-new" style="margin-bottom: 1vh;">MỚI</span> <p class="meos-alert-type">Cảnh báo: '+data.alert.title+'</p> <p class="meos-alert-description">'+data.alert.description+'</p> <hr> <div class="meos-location-button">ĐỊA ĐIỂM</div> </div>';
    } else {
        AlertElement = '<div class="meos-alert" id="alert-'+randId+'"> <span class="meos-alert-new" style="margin-bottom: 1vh;">MỚI</span> <p class="meos-alert-type">Cảnh báo: '+data.alert.title+'</p> <p class="meos-alert-description">'+data.alert.description+'</p></div>';
    }
    $(".meos-recent-alerts").html('<div class="meos-recent-alert" id="recent-alert-'+randId+'"><span class="meos-recent-alert-title">Cảnh báo: '+data.alert.title+'</span><p class="meos-recent-alert-description">'+data.alert.description+'</p></div>');
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData != undefined){
        if (alertData.coords != undefined && alertData.coords != null) {
            ...
            QB.Phone.Notifications.Add("politie", "MDT", "Cảnh báo này không có vị trí GPS!");
        }
    }else {
        QB.Phone.Notifications.Add("politie", "MDT", "Không có cảnh báo nào có sẵn.");
    }
});
...
$(document).on('click', '.meos-clear-alerts', function(e){
    $(".meos-alerts").html("");
    $(".meos-recent-alerts").html('<div class="meos-recent-alert"> <span class="meos-recent-alert-title">Bạn không có bất kỳ cảnh báo nào!</span></div>');
    QB.Phone.Notifications.Add("politie", "MDT", "Tất cả các cảnh báo đã bị xóa!");
});
