...
function SetupPlayerHouses(Houses) {
    ...
    if (Houses.length > 0) {
        $.each(Houses, function(id, house){
            ...
            var elem = '<div class="myhouses-house" id="house-' + id + '"><div class="myhouse-house-icon"><i class="fas fa-home"></i></div> <div class="myhouse-house-titel">' + house.label + ' | Cấp ' + house.tier + '</div> <div class="myhouse-house-details">' + HouseDetails + '</div> </div>';
...
$(document).on('click', '#myhouse-option-transfer-confirm', function(e){
    ...
    $.post('https://qb-phone/TransferCid', JSON.stringify({
        ...
        if (CanTransfer) {
            ...
            QB.Phone.Notifications.Add("fas fa-home", "NHÀ", "Mã công dân không hợp lệ!", "#27ae60", 2500);
