...
local function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            QBCore.Functions.TriggerCallback('xt-garage:server:GetPlayerVehicles', function(vehicles)
                PhoneData.GarageVehicles = vehicles
            end)
        else
            exports['xt-notify']:Alert("HỆ THỐNG", "Bạn không có điện thoại!", 5000, 'error')
        end
    end)
end
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "ĐIỆN THOẠI",
                text = "Bạn không có cuộc gọi đến nào...",
                icon = "fas fa-phone",
                color = "#e84118",
            },
        })
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            exports['xt-notify']:Alert("HỆ THỐNG", "Không thể thực hiện hành động vào lúc này!", 5000, 'error')
           
        end
    end
end)

RegisterKeyMapping('phone', 'Mở điện thoại ( Phím M )', 'keyboard', 'M')
...
RegisterNUICallback('track-vehicle', function(data, cb)
    ...
    if findVehFromPlateAndLocate(veh.plate) then
        exports['xt-notify']:Alert("HỆ THỐNG", "Phương tiện đã được đánh dấu!", 5000, 'success')
    else
        exports['xt-notify']:Alert("HỆ THỐNG", "Phương tiện không thể theo dõi!!", 5000, 'error')
    end
end)
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    exports['xt-notify']:Alert("HỆ THỐNG", "GPS được đặt " ..data.alert.title.. "!", 5000, 'success')
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            exports['xt-notify']:Alert("HỆ THỐNG", "Bạn đã đi xa cuộc đua, GPS được đặt tới đường đua!", 5000, 'error')
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    exports['xt-notify']:Alert("HỆ THỐNG", "GPS được đặt tới " .. data.HouseData.HouseData.adress .. "!", 5000, 'success')
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    exports['xt-notify']:Alert("HỆ THỐNG", "GPS được đặt", 5000, 'success')
end)
...
RegisterCommand('ping', function(source, args)
    local PlayerData = QBCore.Functions.GetPlayerData()
    if not args[1] then
        exports['xt-notify']:Alert("HỆ THỐNG", "Bạn cần nhập ID người chơi!", 5000, 'error')
...
RegisterNetEvent('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "NGÂN HÀNG", text = "&#36;"..amount.." đã được thêm vào số dư!", icon = "fas fa-university", color = "#8c7ae6", }, })
    ...
    if not delete then -- New Tweet
        if src ~= MyPlayerId then
            SendNUIMessage({
                action = "PhoneNotification",
                PhoneNotify = {
                    title = "Tweet mới (@"..NewTweetData.firstName.." "..NewTweetData.lastName..")",
                    text = "Bài tweet đã được đăng.",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                },
            })
...
RegisterNetEvent('qb-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Mail",
            text = "Bạn nhận Mail mới từ "..MailData.sender,
...
RegisterNetEvent('qb-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "QUẢNG CÁO",
            text = "Quảng cáo mới được đăng bởi "..LastAd,
            icon = "fas fa-ad",
            color = "#ff8f1a",
            timeout = 2500,
        },
...
RegisterNetEvent('qb-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "ĐIỆN THOẠI",
            text = "Bạn có liên hệ đề xuất mới!",
            icon = "fa fa-phone-alt",
            color = "#04b543",
            timeout = 1500,
        },
...
RegisterNetEvent('qb-phone:client:GiveContactDetails', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        exports['xt-notify']:Alert("HỆ THỐNG", "Không có ai ở gần!", 5000, 'error')
