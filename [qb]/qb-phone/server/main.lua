...
local function GenerateOwnerName()
    local names = {
        [1] = { name = "Huỳnh Nhật Quân",          citizenid = "DSH091G93" },
        [2] = { name = "Kiều Tấn Trình",        citizenid = "AVH09M193" },
        [3] = { name = "Lương Mạnh Tuấn",            citizenid = "DVH091T93" },
...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    if SenderPly and Config.BillingCommissions[society] then
        ...
        invoiceMailData = {
            sender = 'Thanh Toán Hoá Đơn',
            subject = 'Hoa hồng đã nhận',
            message = string.format('Bạn đã nhận được séc hoa hồng của $%s lúc %s %s đã trả một hóa đơn $%s.', commission, Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    elseif not SenderPly and Config.BillingCommissions[society] then
        invoiceMailData = {
            sender = 'Thanh Toán Hoá Đơn',
            subject = 'Đã thanh toán',
            message = string.format('%s %s đã trả một hóa đơn $%s', Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    end
...
QBCore.Functions.CreateCallback('qb-phone:server:ScanPlate', function(source, cb, plate)
    ...
    if plate ~= nil then
        ...
        TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Không có phương tiện ở gần!", 5000, 'error')
        cb(nil)
    end
end)
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.job.name == "lawyer" or
                ...
                Player.PlayerData.job.name == "doxe" or Player.PlayerData.job.name == "cardealer") and
...
RegisterNetEvent('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    if result[1] ~= nil then
        ...
        TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Số tài khoản không tồn tại!", 5000, 'error')
    end
end)
...
RegisterNetEvent('qb-phone:server:sendPing', function(data)
    ...
    if src ~= data then
        ...
        TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Bạn không thể ping bản thân!", 5000, 'error')
    end
end)
...
QBCore.Commands.Add('bill', 'Bill A Player', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Fine Amount'}}, false, function(source, args)
    ...
    if biller.PlayerData.job.name == "police" or biller.PlayerData.job.name == 'ambulance' or biller.PlayerData.job.name == 'mechanic' then
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('xt-notify:client:Alert', source,"THÔNG BÁO", "Hoá đơn đã được gửi!", 5000, 'success')
                    TriggerClientEvent('xt-notify:client:Alert', billed.PlayerData.source,"THÔNG BÁO", "Đã nhận hoá đơn!", 5000, 'info')
                   
                else
                    TriggerClientEvent('xt-notify:client:Alert', source,"THÔNG BÁO", "Giá trị phải khác 0!", 5000, 'error')
                   
                end
            else
                TriggerClientEvent('xt-notify:client:Alert', source,"THÔNG BÁO", "Bạn không thể tự tính tiền bản thân!", 5000, 'error')
        ...
        TriggerClientEvent('xt-notify:client:Alert', source,"THÔNG BÁO", "Không có truy cập!", 5000, 'error')
