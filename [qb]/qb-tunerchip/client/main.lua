...
RegisterNUICallback('save', function(data)
    QBCore.Functions.TriggerCallback('qb-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            ...
            setVehData(veh, data)
            exports['xt-notify']:Alert("THÔNG BÁO", "Phương tiện đã được hiệU chỉnh", 5000, 'error')
...
RegisterNetEvent('qb-tunerchip:client:TuneStatus', function()
    ...
    if vehModel ~= 0 then
        QBCore.Functions.TriggerCallback('qb-tunerchip:server:GetStatus', function(status)
            if status then
                exports['xt-notify']:Alert("THÔNG BÁO", "Phương tiện đã được hiệu chỉnh", 5000, 'success')
            else
                exports['xt-notify']:Alert("THÔNG BÁO", "Phương tiện chưa được hiệu chỉnh", 5000, 'error')
            end
        end, plate)
    else
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    exports['xt-notify']:Alert("THÔNG BÁO", "Phương tiện đã được cài đặt lại", 5000, 'error')
end)
...
RegisterNetEvent('qb-tunerchip:client:openChip', function()
    ...
    if inVehicle then
        QBCore.Functions.Progressbar("connect_laptop", "Kết nối", 2000, false, true, {
        ...
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn phải ở trong một phương tiện", 5000, 'error')
    end
end)
