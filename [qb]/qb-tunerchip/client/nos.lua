    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            if GetPedInVehicleSeat(veh, -1) == ped then
                QBCore.Functions.Progressbar("use_nos", "Lắp bình Nitro", 1000, false, true, {
                ...
                exports['xt-notify']:Alert("THÔNG BÁO", "Bạn phải ngồi ghế lái", 5000, 'error')
            end
        else
            exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không ở trong phương tiện", 5000, 'error')
        end
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã lắp sẵn NOS", 5000, 'error')
    end
end)
