...
local policeMessage = {
    "Tình huống đáng nghi ngờ",
    "Có thể kinh doanh ma tuý",
}
        ...
        if CurrentCops >= Config.MinimumDrugSalePolice then
            if result ~= nil then
                ...
                if not cornerselling then
                    ...
                    exports['xt-notify']:Alert("THÔNG BÁO", "Bạn bắt đầu bán hoa lá", 5000, 'info')
                    startLocation = GetEntityCoords(PlayerPedId())
                else
                    cornerselling = false
                    LocalPlayer.state:set("inv_busy", false, true)
                    exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã dừng bán hoa lá", 5000, 'error')
                end
            else
                exports['xt-notify']:Alert("THÔNG BÁO", "Không có thì lấy gì mà bán?", 5000, 'error')
                LocalPlayer.state:set("inv_busy", false, true)
            end
        else
            exports['xt-notify']:Alert("THÔNG BÁO", "Không đủ cảnh sát (Cần x"..Config.MinimumDrugSalePolice.." Cảnh sát)", 5000, 'error')
        end
    end)
end)
...
RegisterNetEvent('qb-drugs:client:refreshAvailableDrugs', function(items)
    ...
    if #availableDrugs <= 0 then
        exports['xt-notify']:Alert("THÔNG BÁO", "Không còn đủ hàng", 5000, 'error')
        cornerselling = false
        LocalPlayer.state:set("inv_busy", false, true)
    end
end)
...
local function toFarAway()
    exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đứng quá xa", 5000, 'error')
    LocalPlayer.state:set("inv_busy", false, true)
    cornerselling = false
    hasTarget = false
    startLocation = nil
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                ...
                exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã bị cướp và mất x"..bagAmount.." "..availableDrugs[drugType].label, 5000, 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 and cornerselling then
                    DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, "[~g~E~w~] Bán "..bagAmount.."x "..currentOfferDrug.label.." với giá "..randomPrice.." "..QBCore.Shared.Items["tienban"].label.."? / [~r~G~w~] Từ chối")
                    ...
                    if IsControlJustPressed(0, 47) then
                        exports['xt-notify']:Alert("THÔNG BÁO", "Huỷ giao dịch", 5000, 'error')
                        hasTarget = false
                        SetPedKeepTask(ped, false)
                        SetEntityAsNoLongerNeeded(ped)
                        ClearPedTasksImmediately(ped)
        ...
        if stealingPed ~= nil and stealData ~= nil then
            ...
            if IsEntityDead(stealingPed) then
                ...
                if #(pos - pedpos) < 1.5 then
                    DrawText3D(pedpos.x, pedpos.y, pedpos.z, "[~g~E~s~] - Nhặt")
                    if IsControlJustReleased(0, 38) then
                        ...
                        TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[stealData.item], "add", stealData.amount)
                        stealingPed = nil
                        stealData = {}
                    end
                end
