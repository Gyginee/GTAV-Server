...
RegisterNetEvent('qb-drugs:server:sellCornerDrugs', function(item, amount, price)
    ...
    if Player then
        ...
        if hasItem.amount >= amount then
            TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Chấp nhận thoả thuận", 5000, 'success')
            Player.Functions.RemoveItem(item, amount)
            Player.Functions.AddItem("tienban", price)
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["tienban"], "add", price)
