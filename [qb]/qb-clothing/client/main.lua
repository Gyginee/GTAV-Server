...
Citizen.CreateThread(function()
    for k, v in pairs (Config.Stores) do
        ...
        if Config.Stores[k].shopType == "surgeon" then
            ...
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("Thẩm mỹ viện")
		...
		local dist = #(pos - Config.Stores[k].coords)
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                 DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Shop Quần Áo')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Barber Tóc')
                            elseif Config.Stores[k].shopType == "surgeon" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Viện Thẩm Mỹ')
                            end
                            if IsControlJustPressed(0, 38) then -- E
                                if Config.Stores[k].shopType == "clothing" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "character", label = "Trang Phục", selected = true},
                                        {menu = "accessoires", label = "Phụ Kiện", selected = false}
                                    })
                                elseif Config.Stores[k].shopType == "barber" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "clothing", label = "Tóc", selected = true},
                                    })
                                elseif Config.Stores[k].shopType == "surgeon" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "clothing", label = "Phụ Kiện", selected = true},
                                    })
                                end
                            end
                        end
                    ...
                    if not creatingCharacter then
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                DrawText3Ds(Config.ClothingRooms[k].coords.x, Config.ClothingRooms[k].coords.y, Config.ClothingRooms[k].coords.z + 0.3, '~g~E~w~ - Xem trang phục')
                                if IsControlJustPressed(0, 38) then -- E
                                    ...
                                    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                        openMenu({
                                            {menu = "roomOutfits", label = "Có Sẵn", selected = true, outfits = Config.Outfits[PlayerData.job.name][gender][PlayerData.job.grade.level]},
                                            {menu = "myOutfits", label = "Trang Phục", selected = false, outfits = result},
                                ...
                                if PlayerData.gang.name == Config.ClothingRooms[k].requiredJob then
                                    ...
                                    if IsControlJustPressed(0, 38) then -- E
                                        ...
                                        QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                            openMenu({
                                                {menu = "roomOutfits", label = "Có Sẵn", selected = true, outfits = Config.Outfits[PlayerData.gang.name][gender][PlayerData.gang.grade.level]},
                                                {menu = "myOutfits", label = "Trang Phục", selected = false, outfits = result},
                                                {menu = "character", label = "Nhân Vật", selected = false},
                                                {menu = "accessoires", label = "Phụ Kiện", selected = false}
                                            })
...
AddEventHandler('qb-clothing:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
        openMenu({
            {menu = "myOutfits", label = "Trang Phục Của Tôi", selected = true, outfits = result},
        })
    end)
end)
...
AddEventHandler('qb-clothing:client:openMenu', function()
    ...
    openMenu({
        ...
        {menu = "clothing", label = "Trang Phục", selected = false},
        {menu = "accessoires", label = "Phụ Kiện", selected = false}
    })
end)
...
RegisterNUICallback('TrackerError', function()
    exports['xt-notify']:Alert("HỆ THỐNG", "Bạn không thể gỡ bỏ vòng theo dõi", 5000, 'error')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    ...
    exports['xt-notify']:Alert("HỆ THỐNG", "Bạn đã xoá trang phục: "..data.outfitName.."!", 5000, 'success')
end)
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if oData.outfitName ~= nil then
        exports['xt-notify']:Alert("HỆ THỐNG", "Bạn đã chọn trang phục: "..oData.outfitName.."! Ấn xác nhận để mặc trang phục!", 5000, 'info')
    end
end)
