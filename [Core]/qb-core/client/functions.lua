...
function QBCore.Functions.GetStreetLabel()
...
function QBCore.Functions.GetVehicleProperties(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        local tireHelth = {}
        for i = 0,3 do
            tireHelth[i] = GetVehicleWheelHealth(vehicle, i)
        end
    ...
    if DoesEntityExist(vehicle) then
        ...
        if props.tireBurstState then
            for wheelIndex, burstState in pairs(props.tireBurstState) do
                if burstState then
                    SetVehicleTyreBurst(vehicle, tonumber(wheelIndex), false)
                end
            end
        end
        if props.tireBurstCompletely then
            for wheelIndex, burstState in pairs (props.tireBurstCompletely) do
                if burstState then
                    SetVehicleTyreBurst(vehicle, tonumber(wheelIndex), true)
                end
            end
        end
...
Citizen.CreateThread(function()
    RegisterFontFile('Calistoga-Regular')
    QBCore.FontId = RegisterFontId('Calistoga-Regular')
    AddTextEntry('STRING', "<FONT FACE='Calistoga-Regular'>~a~</FONT>")
    AddTextEntry('CUSTOM_STRING', "<FONT FACE='Calistoga-Regular'>~a~</FONT>")
end)