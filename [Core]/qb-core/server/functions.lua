            ...
            if QBCore.Config.Money.PayCheckSociety then
                ...
                if account ~= 0 then -- Checks if player is employed by a society
                    if account < payment then -- Checks if company has enough money to pay society
                        TriggerClientEvent('xt-notify:client:Alert', Player.PlayerData.source, "THÔNG BÁO", "Công ty của bạn quá <span style='color:#ff0000'>nghèo</span> để trả lương!", 5000, 'error')
                    else
                        Player.Functions.AddMoney('bank', payment)
                        TriggerEvent('qb-bossmenu:server:removeAccountMoney', Player.PlayerData.job.name, payment)
                        TriggerClientEvent('xt-notify:client:Alert', Player.PlayerData.source, "THÔNG BÁO", "Bạn đã nhận được <span style='color:#069a19'><b>"..payment.."$</b></span> tiền lương!", 5000, 'success')
                    end
                else
                    Player.Functions.AddMoney('bank', payment)
                    TriggerClientEvent('xt-notify:client:Alert', Player.PlayerData.source, "THÔNG BÁO", "Bạn đã nhận được <span style='color:#069a19'><b>"..payment.."$</b></span> tiền lương!", 5000, 'success')
...
function QBCore.Functions.Kick(source, reason, setKickReason, deferrals)
    local src = source
    reason = '\n' .. reason .. '\n🔸 Liên hệ để biết thêm: ' .. QBCore.Config.Server.discord
...
function QBCore.Functions.IsPlayerBanned(source)
    ...
    if result then
        if os.time() < result.expire then
            ...
            message = 'Bạn đã bị ban khỏi server:\n' .. result[1].reason .. '\nThời hạn ' .. timeTable.day .. '/' .. timeTable.month .. '/' .. timeTable.year .. ' ' .. timeTable.hour .. ':' .. timeTable.min .. '\n'
        else
