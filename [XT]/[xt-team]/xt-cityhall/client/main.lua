...
CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Toà thị chính")
    ...
    while true do
        ...
        if dist < 20 then
            ...
            if #(pos - vector3(Config.Cityhall.coords.x, Config.Cityhall.coords.y, Config.Cityhall.coords.z)) < 1.5 then
                DrawText3Ds(Config.Cityhall.coords, '~g~E~w~ - Dịch vụ')
...
RegisterNetEvent('xt-cityhall:client:getIds', function()
    TriggerServerEvent('xt-cityhall:server:getIDs')
end)

RegisterNetEvent('xt-cityhall:client:sendDriverEmail', function(charinfo)
...
RegisterNUICallback('requestId', function(data)
    if inRange then
        ...
        TriggerServerEvent('xt-cityhall:server:requestId', idTypes[idType])
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn đã nhận "..idTypes[idType].label.." với giá "..Config.fee.."$ !", 5000, 'success')
    else
        exports['xt-notify']:Alert("THÔNG BÁO", "Không hoạt động", 5000, 'error')
...
RegisterNUICallback('applyJob', function(data)
    if inRange then
        TriggerServerEvent('xt-cityhall:server:ApplyJob', data.job)
    else
