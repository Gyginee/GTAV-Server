...
RegisterNetEvent('xt-cityhall:server:requestId', function(identityData)
    local src = source
    ...
    if  Player.Functions.RemoveMoney("cash", Config.fee) then
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[identityData.item], 'add')
        ...
        TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Bạn không đủ <span style='color:#ff0000'>"..Config.fee.."$ phí</span>!", 5000, 'error')
    end
   
end)

RegisterNetEvent('xt-cityhall:server:getIDs', function()
    local src = source
    GiveStarterItems(src)
end)

RegisterNetEvent('xt-cityhall:server:sendDriverTest', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    for k, v in pairs(DrivingSchools) do
        local SchoolPlayer = QBCore.Functions.GetPlayerByCitizenId(v)
        if SchoolPlayer ~= nil then
            TriggerClientEvent("xt-cityhall:client:sendDriverEmail", SchoolPlayer.PlayerData.source, Player.PlayerData.charinfo)
        else
            local mailData = {
                sender = "Toà Thị Chính",
                subject = "Yêu cầu khoá học bằng lái",
                message = "Chào,<br /><br /> Tôi có nhu cầu học bằng lái từ trung tâm.<br />Nếu có thể, hãy liên hệ tôi:<br />Tên: <strong>".. Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname .. "<br />Số điện thoại: <strong>"..Player.PlayerData.charinfo.phone.."</strong><br/><br/>Trân trọng,<br />",
                button = {}
            }
            TriggerEvent("qb-phone:server:sendNewEventMail", v, mailData)
        end
    end
    TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Email đã gửi tới <span style='color:#ff0000'>trường dạy lái xe</span>!", 5000, 'success')
...
RegisterNetEvent('xt-cityhall:server:ApplyJob', function(job)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if (#(PedCoords - Config.Cityhall.coords) >= 20.0) or (not IsAvailableJob(job)) then
        return DropPlayer(source, "Thử làm một việc không có")
    end

    Player.Functions.SetJob(job, 0)
    TriggerClientEvent('xt-notify:client:Alert', src,"THÔNG BÁO", "Bạn đã xin việc thành công ("..JobInfo.label..") !", 5000, 'info')
end)
...
RegisterNetEvent('xt-cityhall:server:banPlayer', function()
    local src = source
    ...
    DropPlayer(src, 'Cố gắng phá phách')
end)
