...
--		DisplayAmmoThisFrame(false)
		...
		if weapon == GetHashKey("WEAPON_PISTOL") then		
			if IsPedShooting(ped) then
				ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0.025)
				SetWeaponDamageModifier(GetHashKey("WEAPON_PISTOL"), 0.025)
			end
		end
		if weapon == GetHashKey("weapon_machete") then
			if IsPedShooting(ped) then
		...
		if weapon == GetHashKey("WEAPON_ASSAULTRIFLE") then			
			if IsPedShooting(ped) then
				ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0)
				SetWeaponDamageModifier(GetHashKey("WEAPON_ASSAULTRIFLE"), 0.9)
			end
		end
		...
		if weapon == GetHashKey("WEAPON_ASSAULTRIFLE_MK2") then			
			if IsPedShooting(ped) then
				ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0)
				SetWeaponDamageModifier(GetHashKey("WEAPON_ASSAULTRIFLE"), 1)
			end
		end
		...
		if weapon == GetHashKey("WEAPON_CARBINERIFLE") then	
			if IsPedShooting(ped) then
				ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0)
				SetWeaponDamageModifier(GetHashKey("WEAPON_CARBINERIFLE"), 0.99)
			end
		end
