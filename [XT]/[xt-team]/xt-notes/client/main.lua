        ...
        if closestNote ~= 0 then
            if Notes[closestNote] ~= nil and not Notes[closestNote].active then
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Notes[closestNote].coords.x, Notes[closestNote].coords.y, Notes[closestNote].coords.z, true) < 1.5 then
                    DrawText3D(Notes[closestNote].coords.x, Notes[closestNote].coords.y, Notes[closestNote].coords.z, "[~g~E~w~] - Đọc [~g~G~w~] - Vứt")
...
RegisterNetEvent("notes:client:OpenNotepad", function(noteId, text)
...
RegisterNetEvent("notes:client:SetActiveStatus", function(noteId, status)
    Notes[noteId].active = status
end)

RegisterNetEvent("notes:client:AddNoteDrop", function(noteId, player)
    local ped =PlayerPedId()
...
RegisterNetEvent("notes:client:RemoveNote", function(noteId)
    Notes[noteId] = nil
