...
RegisterNetEvent('qb-carwash:client:washCar', function()
    ...
    washingVehicle = true
    QBCore.Functions.Progressbar("search_cabin", "Rửa phương tiện", math.random(4000, 8000), false, true, {
                    ...
                    if dist <= 7.5 then
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                DrawText3Ds(Config.CarWash[k]["coords"]["x"], Config.CarWash[k]["coords"]["y"], Config.CarWash[k]["coords"]["z"], 'Nhấn [~g~E~w~] - Rửa xe ($'..Config.DefaultPrice..')')
                                if IsControlJustPressed(0, 38) then
                                    if dirtLevel > Config.DirtLevel then
                                        TriggerServerEvent('qb-carwash:server:washCar')
                                    else
                                        exports['xt-notify']:Alert("THÔNG BÁO", "Phương tiện này không <span style='color:#fc1100'>bẩn</span>!", 5000, 'error')
                                    end
                                end
                            else
                                DrawText3Ds(Config.CarWash[k]["coords"]["x"], Config.CarWash[k]["coords"]["y"], Config.CarWash[k]["coords"]["z"], 'Tiệm rửa xe đang bận')
                            end
                        end
                    end
                end
