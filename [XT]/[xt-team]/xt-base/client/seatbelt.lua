...
RegisterNetEvent('seatbelt:client:UseHarness', function(ItemData) -- On Item Use (registered server side)
    ...
    if inveh and class ~= 8 and class ~= 13 and class ~= 14 then
        if not harnessOn then
            ...
            QBCore.Functions.Progressbar("harness_equip", "Thắt dây an toàn", 5000, false, true, {
            ...
            LocalPlayer.state:set("inv_busy", true, true)
            QBCore.Functions.Progressbar("harness_equip", "Tháo dây an toàn", 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
        ...
        exports['xt-notify']:Alert("THÔNG BÁO", "Bạn không trong xe hơi", 5000, 'error')
    end
end)
...
function EjectFromVehicle()
    ...
    local ejectspeed = math.ceil(GetEntitySpeed(ped) * 1)
