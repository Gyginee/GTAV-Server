...
QBCore.Functions.CreateUseableItem("sandwich", function(source, item)
	...
	if Player.Functions.RemoveItem(item.name, 1, item.slot) then
        TriggerClientEvent("consumables:client:Eat", src, item.name , 'sandwich')
    end
end)
...
QBCore.Functions.CreateUseableItem("twerks_candy", function(source, item)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
	if Player.Functions.RemoveItem(item.name, 1, item.slot) then
        TriggerClientEvent("consumables:client:Eat", src, item.name, 'twerks_candy')
    end
end)
...
QBCore.Functions.CreateUseableItem("water_bottle", function(source, item)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
	if Player.Functions.RemoveItem(item.name, 1, item.slot) then
        TriggerClientEvent("consumables:client:Drink", src, item.name, 'water')
    end
end)
...
QBCore.Functions.CreateUseableItem("coffee", function(source, item)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
	if Player.Functions.RemoveItem(item.name, 1, item.slot) then
        TriggerClientEvent("consumables:client:Drink", src, item.name, 'coffee')
    end
end)

QBCore.Functions.CreateUseableItem("cola", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	if Player.Functions.RemoveItem(item.name, 1, item.slot) then
        TriggerClientEvent("consumables:client:Drink", src, item.name, 'cola')
    end
end)
