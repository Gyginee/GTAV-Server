...
local function lockpickFinish(success)
	if success then
		exports['xt-notify']:Alert("HỆ THỐNG", "Thành công'", 5000, 'success')
		...
		Wait(1800)
		TriggerServerEvent('xt-doorlock:server:updateState', closestDoor.id, false, false, true, false) -- Broadcast new state of the door to everyone
	else
		exports['xt-notify']:Alert("HỆ THỐNG", "Thất bại'", 5000, 'error')
...
local function isAuthorized(door)
	...
	if door.items then
		local p = promise.new()
		QBCore.Functions.TriggerCallback('xt-doorlock:server:checkItems', function(result)
			p:resolve(result)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	QBCore.Functions.TriggerCallback('xt-doorlock:server:setupDoors', function(result)
...
RegisterNetEvent('xt-doorlock:client:setState', function(serverId, doorID, state, src, enableAnimation, enableSounds)
...
RegisterNetEvent('xt-doorlock:client:addNewDoor', function()
	canContinue = false
	hideNUI()
	local doorData = {}
	local dialog = exports['qb-input']:ShowInput({
		header = "Thêm khoá cửa mới",
		submitText = "Xác nhận",
		inputs = {
			{
				text = "Tên file Config",
				name = "configfile",
				type = "text",
				isRequired = false
			},
			...
			},
			{
				...
				type = "select",
				options = {
					{ value = "door", text = "Cửa đơn" },
					{ value = "double", text = "Cửa đôi" },
					{ value = "sliding", text = "Cửa trượt đơn" },
					{ value = "doublesliding", text = "Cửa trượt đôi" },
			...
			},
			{
				...
				type = "checkbox",
				options = {
					{ value = "locked", text = "Đã khoá" },
					{ value = "pickable", text = "Có thể phá khoá" }
				}
			}
		}
	})
	...
	if doorData.doortype == 'door' or doorData.doortype == 'sliding' or doorData.doortype == 'garage' then
		...
		if not model or model == 0 then exports['xt-notify']:Alert("HỆ THỐNG", "Không tìm thấy cửa'", 5000, 'error') canContinue = true return end
		result = DoorSystemFindExistingDoor(coords.x, coords.y, coords.z, model)
		if result then exports['xt-notify']:Alert("HỆ THỐNG", "Cửa này đã được tạo'", 5000, 'error') canContinue = true return end
		...
		TriggerServerEvent('xt-doorlock:server:saveNewDoor', doorData, false)
		canContinue = true
	else
		...
		if not model[1] or model[1] == 0 or not model[2] or model[2] == 0 then exports['xt-notify']:Alert("HỆ THỐNG", "Không tìm thấy cửa'", 5000, 'error') return end
		if entity[1] == entity[2] then exports['xt-notify']:Alert("HỆ THỐNG", "Trùng nhau'", 5000, 'error') canContinue = true return end
		doorData.doorHash = {}
		for i = 1, 2 do
			result = DoorSystemFindExistingDoor(coords[i].x, coords[i].y, coords[i].z, model[i])
			if result then exports['xt-notify']:Alert("HỆ THỐNG", "Cửa đã được tạo'", 5000, 'error') canContinue = true return end
		...
		doorData.model = model
		doorData.heading = heading
		TriggerServerEvent('xt-doorlock:server:saveNewDoor', doorData, true)
		canContinue = true
	end
end)

RegisterNetEvent('xt-doorlock:client:newDoorAdded', function(data, id)
	Config.DoorList[id] = data
	TriggerEvent('xt-doorlock:client:setState', PlayerData.source, id, data.locked, false, true, true)
end)
...
RegisterCommand('toggledoorlock', function()
	...
	TriggerServerEvent('xt-doorlock:server:updateState', closestDoor.id, locked, src, false, false, true, true) -- Broadcast new state of the door to everyone
end)
TriggerEvent("chat:removeSuggestion", "/toggledoorlock")
RegisterKeyMapping('toggledoorlock', "Tương tác với khóa cửa", 'keyboard', 'E')
