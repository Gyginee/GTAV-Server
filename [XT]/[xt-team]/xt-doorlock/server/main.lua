...
local function showWarning(msg)
	print(('^3%s: %s^0'):format("Warning", msg))
end
...
local function isAuthorized(Player, door, usedLockpick)
	...
	if Config.AdminAccess and QBCore.Functions.HasPermission(Player.PlayerData.source, Config.AdminPermission) then
		if Config.Warnings then
			showWarning(Player.PlayerData.name.." license:"..Player.PlayerData.license.." đã mở cửa với quyền quản trị viên")
		end
		return true
	end
...
QBCore.Functions.CreateCallback('xt-doorlock:server:setupDoors', function(_, cb)
	cb(Config.DoorList)
end)

QBCore.Functions.CreateCallback('xt-doorlock:server:checkItems', function(source, cb, items, needsAll)
...
RegisterNetEvent('xt-doorlock:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	if type(doorID) ~= 'number' and type(doorID) ~= 'string' then
		if Config.Warnings then
			...
			showWarning(player.." "..license.." cố gắng cập nhật cửa không hợp lệ (Đến: "..doorID..")")
		end
		return
	end
	...
	if type(locked) ~= 'boolean' then
		if Config.Warnings then
			local player = Player.PlayerData.name
			local license = Player.PlayerData.license
			showWarning(player.." "..license.." đã cố gắng cập nhật lên trạng thái không hợp lệ (Đến: "..locked..")")
		end
		return
	end
	...
	if not unlockAnyway and not isAuthorized(Player, Config.DoorList[doorID], usedLockpick) then
		if Config.Warnings then
			local player = Player.PlayerData.name
			local license = Player.PlayerData.license
			showWarning(player..""..license.." cố gắng mở cửa mà không được phép (Đến: "..doorID..")")
		end
		return
	end

	Config.DoorList[doorID].locked = locked
	TriggerClientEvent('xt-doorlock:client:setState', -1, playerId, doorID, locked, src or false, enableSounds, enableAnimation)
	...
	SetTimeout(Config.DoorList[doorID].autoLock, function()
		...
		TriggerClientEvent('xt-doorlock:client:setState', -1, playerId, doorID, true, src or false, enableSounds, enableAnimation)
	end)
end)

RegisterNetEvent('xt-doorlock:server:saveNewDoor', function(data, doubleDoor)
	local src = source
	if not QBCore.Functions.HasPermission(src, Config.CommandPermission) and not IsPlayerAceAllowed(src, 'command') then
		if Config.Warnings then
			showWarning(GetPlayerName(src).." license: "..QBCore.Functions.GetIdentifier(src, 'license').." đã thử add của khi không có quyền (source: "..src..")")
		end
		return
	end
	...
	local label = "\n\n-- "..data.doorname.." ".. "tạo bởi" .." "..Player.PlayerData.name.."\nConfig.DoorList['"..data.doorname.."'] = {"
	file:write(label)
	...
	TriggerClientEvent('xt-doorlock:client:newDoorAdded', -1, configData, data.doorname)
end)

-- Commands

QBCore.Commands.Add('newdoor', "Add khoá cửa", {}, false, function(source)
	TriggerClientEvent('xt-doorlock:client:addNewDoor', source)
