...
local function saveVehicle()
    ...
    TriggerServerEvent('xt-doxe:server:updateVehicle', myCar)
end
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("xt-doxe:server:attemptPurchase", type, upgradeLevel)
...
function EnterLocation(override)
    ...
    local welcomeLabel = (locationData and locationData.settings.welcomeLabel) or "Chào mừng bạn đến với Xưởng độ xe X-Team!"
    InitiateMenus(isMotorcycle, categories, welcomeLabel)
...
function GetLocations()
    QBCore.Functions.TriggerCallback("xt-doxe:server:GetLocations", function(locations)
    ...
    if Config.UseRadial then
        if not radialMenuItemId then
            radialMenuItemId = exports['qb-radialmenu']:AddOption({
                ...
                icon = 'wrench',
                type = 'client',
                event = 'xt-doxe:client:EnterCustoms',
                shouldClose = true
            })
        end
    else
...
RegisterNetEvent('xt-doxe:client:UpdateLocation', function(location, type, key, value)
    Config.Locations[location][type][key] = value
end)

RegisterNetEvent("xt-doxe:client:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    exports['xt-notify']:Alert("ĐỘ XE", "Đã lắp", 5000, 'success')
end)

RegisterNetEvent("xt-doxe:client:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    exports['xt-notify']:Alert("ĐỘ XE", "Không đủ tiền", 5000, 'error')
end)

RegisterNetEvent('xt-doxe:client:EnterCustoms', function(override)
