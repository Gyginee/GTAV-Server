...
function InitiateMenus(isMotorcycle, categories, welcomeLabel)
    local plyPed = PlayerPedId()
    local plyVeh = GetVehiclePedIsIn(plyPed, false)
    --#[Repair Menu]#--

    --#[Main Menu]#--
    createMenu("mainMenu", welcomeLabel, "Chọn một danh mục")
    ...
    for k, v in ipairs(vehicleCustomisation) do
        ...
        if amountValidMods > 0 or v.id == 18 then
            if v.id == 13 or v.id == 15 then --Performance Upgrades
                local tempNum = 0

                createMenu(v.category:gsub("%s+", "") .. "Menu", v.category, "Chọn nâng cấp")
                ...
                for m, n in pairs(validMods) do
                    ...
                    if maxVehiclePerformanceUpgrades == 0 then
                        ...
                        if currentMod == n.id then
                            updateItem2Text(v.category:gsub("%s+", "") .. "Menu", n.id, "Đã lắp")
                        end
                    else
            ...
            elseif v.id == 11  then --Performance Upgrades
                    local tempNum = 0
                    ...
                    for m, n in pairs(validMods) do
                        ...
                        if maxVehiclePerformanceUpgrades == 0 then
                            populateMenu(v.category:gsub("%s+", "") .. "Menu", n.id, n.name, "$" .. vehicleCustomisationPrices.engine.prices[tempNum])
                    ...
                    finishPopulatingMenu(v.category:gsub("%s+", "") .. "Menu")
             elseif v.id == 12  then --Performance Upgrades
                    local tempNum = 0
                    ...
                    for m, n in pairs(validMods) do
                        ...
                        if maxVehiclePerformanceUpgrades == 0 then
                            populateMenu(v.category:gsub("%s+", "") .. "Menu", n.id, n.name, "$" .. vehicleCustomisationPrices.brake.prices[tempNum])
                    ...
                    finishPopulatingMenu(v.category:gsub("%s+", "") .. "Menu")
                elseif v.id == 16  then --Performance Upgrades
                    local tempNum = 0
                    ...
                    for m, n in pairs(validMods) do
                        ...
                        if maxVehiclePerformanceUpgrades == 0 then
                            populateMenu(v.category:gsub("%s+", "") .. "Menu", n.id, n.name, "$" .. vehicleCustomisationPrices.armor.prices[tempNum])
            ...
            elseif v.id == 18 then
                ...
                createMenu(v.category:gsub("%s+", "") .. "Menu",  "Điều chỉnh "..v.category, "Lắp/Tháo Turbo")
                ...
                updateItem2Text(v.category:gsub("%s+", "") .. "Menu", currentTurboState, "Đã lắp")

                finishPopulatingMenu(v.category:gsub("%s+", "") .. "Menu")
            else
                createMenu(v.category:gsub("%s+", "") .. "Menu", "Điều chỉnh "..v.category, "Chọn 1 Option")
    ...
    --#[Respray Menu]#--
    createMenu("ResprayMenu", "Màu sơn", "Chọn vị trí sơn")
    ...
    --#[Respray Types]#--
    createMenu("ResprayTypeMenu", "Màu sơn", "Chọn loại sơn")
    ...
    --#[Respray Colours]#--
    for k, v in ipairs(vehicleResprayOptions) do
        createMenu(v.category .. "Menu",  "Màu "..v.category, "Chọn 1 màu")
    ...
    createMenu("WheelsMenu", "Bánh xe", "Chọn một danh mục")
    ...
    for k, v in ipairs(vehicleWheelOptions) do
        if v.id == -1 then
            ...
            createMenu(v.category:gsub("%s+", "") .. "Menu", v.category, "Lắp/Tháo bánh xe")

            populateMenu(v.category:gsub("%s+", "") .. "Menu", 0, "Tháo", "$0")
            populateMenu(v.category:gsub("%s+", "") .. "Menu", 1, "Lắp", "$" .. vehicleCustomisationPrices.customwheels.price)

            updateItem2Text(v.category:gsub("%s+", "") .. "Menu", currentCustomWheelState, "Đã lắp")
        ...
        elseif v.id ~= 20 then
            if isMotorcycle then
                if v.id == 6 then --Motorcycle Wheels
                    ...
                    createMenu(v.category .. "Menu",  "Bánh xe "..v.category, "Chọn một bánh xe")
    ...
    createMenu("TyreSmokeMenu", "Tùy chỉnh khói", "Chọn một màu")
    ...
    for k, v in ipairs(vehicleTyreSmokeOptions) do
        ...
        if v.r == currentWheelSmokeR and v.g == currentWheelSmokeG and v.b == currentWheelSmokeB then
            updateItem2Text("TyreSmokeMenu", k, "Đã lắp")
        end
    end
    ...
    createMenu("WindowTintMenu", "Tuỳ chỉnh màu kính", "Chọn một màu")
    ...
    for k, v in ipairs(vehicleWindowTintOptions) do
        ...
        if currentWindowTint == v.id then
            updateItem2Text("WindowTintMenu", v.id, "Đã lắp")
        end
    end
    ...
    if livCount > 0 then
        ...
        createMenu("OldLiveryMenu", "Skin xe", "Chọn skin")
        for i=0, livCount-1 do
            populateMenu("OldLiveryMenu", i, "Skin xe", "$100")
            if tempOldLivery == i then
                updateItem2Text("OldLiveryMenu", i, "Đã lắp")
            end
        end
    ...
    createMenu("PlateIndexMenu", "Biển số", "Chọn một kiểu")
    local plateTypes = {
        "Blue on White #1",
        "Yellow on Black",
        "Yellow on Blue",
    ...
    for i=0, #plateTypes-1 do
        if i ~= 4 or (i == 4 and GetVehicleClass(plyVeh) == 18) or Config.allowGovPlateIndex then
            ...
            if tempPlateIndex == i then
                updateItem2Text("PlateIndexMenu", i, "Đã lắp")
            end
        end
    end
    ...
    --#[Vehicle Extras Menu]#--
    createMenu("VehicleExtrasMenu", "Tùy chỉnh bổ sung xe", "Chuyển đổi các tính năng bổ sung")
    for i=1, 12 do
        if DoesExtraExist(plyVeh, i) then
            populateMenu("VehicleExtrasMenu", i, "Bổ sung "..tostring(i), "Toggle")
        else
            populateMenu("VehicleExtrasMenu", i, "Không có tùy chọn", "NONE")
        end
    end
    finishPopulatingMenu("VehicleExtrasMenu")

    --#[Neons Menu]#--
    createMenu("NeonsMenu", "Đèn gầm", "Chọn một danh mục")
    ...
    for k, v in ipairs(vehicleNeonOptions.neonTypes) do
        ...
        createMenu(v.name:gsub("%s+", "") .. "Menu", "Đèn gầm", "Tháo/Lắp đèn gầm")

        populateMenu(v.name:gsub("%s+", "") .. "Menu", 0, "Tháo", "$0")
        populateMenu(v.name:gsub("%s+", "") .. "Menu", 1, "Lắp", "$" .. vehicleCustomisationPrices.neonside.price)

        updateItem2Text(v.name:gsub("%s+", "") .. "Menu", currentNeonState, "Đã lắp")
    ...
    createMenu("NeonColoursMenu", "Màu", "Chọn một màu")
    ...
    for k, v in ipairs(vehicleNeonOptions.neonColours) do
        ...
        if currentNeonR == vehicleNeonOptions.neonColours[k].r and currentNeonG == vehicleNeonOptions.neonColours[k].g and currentNeonB == vehicleNeonOptions.neonColours[k].b then
            updateItem2Text("NeonColoursMenu", k, "Đã lắp")
        end
    end
  ...
  createMenu("HeadlightsMenu", "Đèn led", "Tháo/Lắp đèn led")

  populateMenu("HeadlightsMenu", 0, "Tháo", "$0")
  populateMenu("HeadlightsMenu", 1, "Lắp", "$" .. vehicleCustomisationPrices.headlights.price)

  updateItem2Text("HeadlightsMenu", currentXenonState, "Đã lắp")

  finishPopulatingMenu("HeadlightsMenu")

  --#[Xenons Colour Menu]#--
  local currentXenonColour = GetCurrentXenonColour()
  createMenu("XenonColoursMenu", "Đèn led", "Chọn 1 màu")
...
function MenuManager(state, repairOnly)
    if state then
        if currentMenuItem2 ~= "Đã lắp" then
            if isMenuActive("modMenu") then
                if currentCategory == 18 then --Turbo
                    if AttemptPurchase("turbo", currentMenuItemID) then
                        ...
                        updateItem2Text(currentMenu, currentMenuItemID, "Đã lắp")
                        updateMenuStatus("Lắp thành công")
                    else
                        updateMenuStatus("Không đủ tiền!")
                    end
                elseif currentCategory== 13 or currentCategory == 15 then --Performance Upgrades
                ...
                elseif currentCategory == 11  then --Performance Upgrades
                    if AttemptPurchase("engine", currentMenuItemID) then
                        ApplyMod(currentCategory, currentMenuItemID)
                        playSoundEffect("wrench", 0.4)
                        updateItem2Text(currentMenu, currentMenuItemID, "Đã lắp")
                        updateMenuStatus("Lắp thành công")
                ...
                elseif currentCategory == 12  then --Performance Upgrades
                ...
                elseif currentCategory == 16  then --Performance Upgrades
                    if AttemptPurchase("armor", currentMenuItemID) then
                        ApplyMod(currentCategory, currentMenuItemID)
                        playSoundEffect("wrench", 0.4)
                        updateItem2Text(currentMenu, currentMenuItemID, "Đã lắp")
                        updateMenuStatus("Lắp thành công")
                ...
                if currentWheelCategory == 20 then
                    ...
                    if currentWheelCategory == -1 then --Custom Wheels
                        ...
                        if currentWheel == -1 then
                            updateMenuStatus("Không thể áp dụng lốp tùy chỉnh cho bánh xe có sẵn")
                        else
                        ...
                        if currentCustomWheelState and currentWheel == -1 then
                            updateMenuStatus("Không thể áp dụng bánh xe có sẵn với lốp tùy chỉnh")
                        else
            ...
            elseif isMenuActive("NeonsSideMenu") then
                ...
                elseif currentMenu == "WheelsMenu" then
                    ...
                    if currentWheelType == currentWheelCategory then
                        updateItem2Text(currentMenu, currentWheel, "Đã lắp")
                    end
