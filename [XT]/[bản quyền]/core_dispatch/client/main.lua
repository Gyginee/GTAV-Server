...
AddEventHandler("core_dispatch:arrivalNoticeClient", function()
    if not unitCooldown then
        exports['xt-notify']:Alert("TRUNG ƯƠNG", "Đơn của bạn đã được nhận!", 5000, 'success')
        TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5, "mon", 0.5)
        unitCooldown = true
        Citizen.Wait(20000)
        unitCooldown = false
    end
...
RegisterNUICallback("reqbackup", function(data)
    ...
    exports['xt-notify']:Alert("TRUNG ƯƠNG", "Yêu cầu hỗ trợ!", 5000, 'warning') 
    local cord = GetEntityCoords(PlayerPedId())
    TriggerServerEvent("core_dispatch:addCall",
        "00-00",
        req .. " đang yêu cầu trợ giúp",
...
RegisterNUICallback(
    ...
    function(data)
        ...
        if unitBlipsToggled then
            addBlipsForUnits()
            exports['xt-notify']:Alert("TRUNG ƯƠNG", "Các đơn vị đã được hiển thị", 5000, 'info') 
        else
            for _, z in pairs(blips) do
                RemoveBlip(z)
            end
            ...
            blips = {}
            exports['xt-notify']:Alert("TRUNG ƯƠNG", "Các đơn vị đã được ẩn", 5000, 'warning')
...
RegisterNUICallback(
    ...
    function(data)
        ...
        if alertsToggled then
            exports['xt-notify']:Alert("TRUNG ƯƠNG", "Cảnh báo đã được bật", 5000, 'info')
            -- SendTextMessage(Config.Text["alerts_turned_on"])
        else
            exports['xt-notify']:Alert("TRUNG ƯƠNG", "Cảnh báo đã được tắt", 5000, 'warning')
...
RegisterNUICallback(
    "copynumber",
    function(data)
        exports['xt-notify']:Alert("TRUNG ƯƠNG", "Số điện thoại đã được sao lưu", 5000, 'info')
...
RegisterNUICallback(
    ...
    function(data)
        local id = data["id"]:gsub("call_", "")
        exports['xt-notify']:Alert("TRUNG ƯƠNG", "Cuộc gọi được chuyển tiếp", 5000, 'info')
...
AddEventHandler("core_dispatch:acceptCallClient", function(id, unit)
    if unit == GetPlayerServerId(PlayerId()) then
        exports['xt-notify']:Alert("TRUNG ƯƠNG", "Chấp nhận", 5000, 'success')
    end
...
AddEventHandler("core_dispatch:removeCallClient", function(id, unit)
    if unit == GetPlayerServerId(PlayerId()) then
        exports['xt-notify']:Alert("TRUNG ƯƠNG", "Cuộc gọi được hủy", 5000, 'error')
    end
    SendNUIMessage(
        {
            type = "remove",
