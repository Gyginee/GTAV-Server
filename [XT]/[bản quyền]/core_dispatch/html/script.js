  ...
  if (element != null) {
    ...
    if (columnMenu) {
      ...
      if (jobInfo[job].canRequestLocalBackup && col == jobInfo[job].column) {
        html = html + '    <a onclick="requestBackup()">Yêu cầu hỗ trợ</a>';
      }
      ...
      html = html + '    <a onclick="changeToBusy()">Chuyển sang Bận</a>';
      html = html + '    <a onclick="changeToPatrol()">Chuyển sang Làm việc</a>';


    }
    else {
      if (acceptedCall == '') {
        html = html + '    <a onclick="acceptCall()">Nhận cuộc gọi</a>';
        if (jobInfo[job].canRemoveCall) {
          html = html + '    <a onclick="removeCall()">Xóa cuộc gọi</a>';
        }
        if ($(element).hasClass('message')) {
          html = html + '    <a onclick="copyNumber()">Sao chép số</a>';
        }

      }

      if (acceptedCall != '' && acceptedCall == $(element).attr('id')) {
        html = html + '    <a onclick="dismissCall()">Từ chối cuộc gọi</a>';
      }


      for (const [key, value] of Object.entries(jobInfo)) {
        if (key != job && jobInfo[job].forwardCall) {
          html = html + '    <a onclick="forwardCall(' + value.column + ')">Chuyển Tới  ' + value.label + '</a>';
        }
      }
