...
RegisterNetEvent('okokContract:GetVehicleInfo', function(source_playername, date, description, price, source)
	...
	if closestPlayer ~= -1 and playerDistance <= 3.0 then
		...
		if DoesEntityExist(vehicle) and (vehDistance <= 3) then
			...
			exports['xt-notify']:Alert("BÁN XE", "Bạn cần ở gần người khác để làm hợp đồng", 5000, 'error')
		end
	else
		ClearPedTasks(PlayerPedId())
		exports['xt-notify']:Alert("BÁN XE", "Bạn cần ở gần người khác để làm hợp đồng ", 5000, 'error')
...
RegisterNetEvent('okokContract:OpenContractInfo', function()
	SetNuiFocus(true, true)
	SendNUIMessage({
...
RegisterNetEvent('okokContract:OpenContractOnBuyer', function(data)
	SetNuiFocus(true, true)
	SendNUIMessage({
...
RegisterNetEvent('okokContract:startContractAnimation', function(player)
