        ...
        $("#keymaster-field").fadeIn(500, () => {
            ...
            let cd = window.setInterval(() => {
                if(countdown == -1){
                    ...
                    $("#keymaster-countdown").html("BẮT ĐẦU");
                    countdown--;
...
function endGame(status, message){
    ...
    if(settings.handleEnd){
        openModal(((status)?"Thành công!":"Thất bại!"), message);
    } else {
...
async function evalStatus(){
    if(score >= settings.scoreWin){
        endGame(true, "Thành công");
        return;
    } 
    if(score <= settings.scoreLose){
        endGame(false, "Không đủ điểm!");
        return;
    } 
    if(mistake > settings.maxMistake){
        endGame(false, `Vượt quá ${settings.maxMistake} lỗi.`);
        return
    }
}
