...
local Webhook = 'https://discord.com/api/webhooks/917368295437500436/jPH-dzWNf5tXk00_fdrx72IR7Fv0osyJi5-nVTtQducZ4sScb1jbbrakG869ZQIQJeaO'
...
RegisterServerEvent("okokBilling:PayInvoice", function(invoice_id)
	local xPlayer = QBCore.Functions.GetPlayer(source)
	local _source = source
	...
	MySQL.Async.fetchAll('SELECT * FROM okokBilling WHERE id = @id', {
		...
		if playerMoney < invoices.invoice_value then
			TriggerClientEvent('xt-notify:client:Alert', _source, "HÓA ĐƠN", "Bạn không có đủ tiền!", 5000, 'error')
		else
			...
			MySQL.Async.fetchAll('UPDATE okokBilling SET status = @status, paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
				['@status'] = 'paid',
				['@id'] = invoice_id
			})

			TriggerClientEvent('xt-notify:client:Alert', _source, "HÓA ĐƠN", "Đã thanh toán hóa đơn thành công!", 5000, 'success')
...
RegisterServerEvent("okokBilling:CancelInvoice", function(invoice_id)
	local xPlayer = QBCore.Functions.GetPlayer(source)
	local _source = source
	...
	MySQL.Async.fetchAll('SELECT * FROM okokBilling WHERE id = @id', {
		...
		MySQL.Async.fetchAll('UPDATE okokBilling SET status = "cancelled", paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
			['@id'] = invoice_id
		})
		TriggerClientEvent('xt-notify:client:Alert', _source, "HÓA ĐƠN", "Đã hủy hóa đơn!", 5000, 'info')
		if Webhook ~= '' then
...
RegisterServerEvent("okokBilling:CreateInvoice", function(data)
	...
	if Config.LimitDate then
		MySQL.Async.insert('INSERT INTO okokBilling (receiver_identifier, receiver_name, author_identifier, author_name, society, society_name, item, invoice_value, status, notes, sent_date, limit_pay_date) VALUES (@receiver_identifier, @receiver_name, @author_identifier, @author_name, @society, @society_name, @item, @invoice_value, @status, @notes, CURRENT_TIMESTAMP(), DATE_ADD(CURRENT_TIMESTAMP(), INTERVAL @limit_pay_date DAY))', {
			...
			TriggerClientEvent('xt-notify:client:Alert', target.PlayerData.source, "HÓA ĐƠN", "Bạn nhận được một hóa đơn mới!", 10000, 'info')
			if Webhook ~= '' then
					...
					for k, vl in pairs(whenToAddFees) do
						if v.fees_amount == k - 1 then
							if v.timeLeft >= vl*(-1) then
								MySQL.Async.fetchAll('UPDATE okokBilling SET fees_amount = @fees_amount, invoice_value = @invoice_value WHERE id = @id', {
									['@fees_amount'] = k,
				...
				elseif v.timeLeft >= 0 and Config.PayAutomaticallyAfterLimit then
					...
					if xPlayer == nil then
						MySQL.Async.fetchAll('SELECT money FROM players WHERE citizenid = @id', {
							...
							local playerAccount = json.decode(account[0].money)
							...
							MySQL.Async.fetchAll('UPDATE players SET money = @playerAccount WHERE citizenid = @target', {
								...
								MySQL.Async.fetchAll('UPDATE okokBilling SET status = @paid, paid_date = CURRENT_TIMESTAMP() WHERE id = @id', {
									['@paid'] = 'autopaid',
									['@id'] = v.id
								})
							end)
