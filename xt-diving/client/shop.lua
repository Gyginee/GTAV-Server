        ...
        if not notInteressted then
            for k, v in pairs(QBDiving.SellLocations) do
                ...
                if dist < 20 then
                    inRange = true
                    if dist < 1 then
                        DrawText3D(v["coords"]["x"], v["coords"]["y"], v["coords"]["z"] - 0.1, '~g~G~w~ - Bán san hô')
                        if IsControlJustPressed(0, 47) then
                            LocalPlayer.state:set("inv_busy", true, true)
                            TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_STAND_IMPATIENT", 0, true)
                            QBCore.Functions.Progressbar("sell_coral_items", "Kiểm tra túi để bán san hô ", math.random(2000, 4000), false, true, {}, {}, {}, {}, function() -- Done
                                ...
                                ClearPedTasks(PlayerPedId())
                                exports['okokNotify']:Alert("HỆ THỐNG", "Đã hủy ", 5000, 'error')
