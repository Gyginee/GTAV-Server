    ...
    while true do
        ...
        if distance < 15 then
            ...
            if BuyDistance < 2 then                
                ...
                if not Buying then
                    DrawText3D(BuyLocation.x, BuyLocation.y, BuyLocation.z + 0.3, '~g~E~w~ - '..QBBoatshop.ShopBoats[currentBoat]["label"]..' với giá ~b~$'..QBBoatshop.ShopBoats[currentBoat]["price"])
                    if IsControlJustPressed(0, 38) then
                        Buying = true
                    end
                else
                    DrawText3D(BuyLocation.x, BuyLocation.y, BuyLocation.z + 0.3, 'Bạn có chắc chắn ~g~7~w~ Có / ~r~8~w~ Không ~b~($'..QBBoatshop.ShopBoats[currentBoat]["price"]..',-)')
...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Mua thuyền")
