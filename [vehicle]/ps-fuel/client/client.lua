...
RegisterNetEvent('ps-fuel:client:buyCanMenu', function()
	exports['qb-menu']:openMenu({
		{
			header = "Petrolimex",
			txt = 'Tổng tiền phải chi trả là : $'..Config.canCost..' ghi tại hoá đơn.',
			params = {
...
RegisterNetEvent('ps-fuel:client:refuelCanMenu', function()
	...
	if fuelToAdd ~= 0 then
		...
		exports['qb-menu']:openMenu({
			{
				header = "Petrolimex",
				txt = "Tổng tiền phải chi trả là "..Config.refuelCost.."$",
				params = {
		...
		exports['xt-notify']:Alert("THÔNG BÁO", "Can xăng đầy", 5000, 'error')
...
RegisterNetEvent('ps-fuel:client:refuelCan', function()
	...
	if HasPedGotWeapon(ped, 883325847) then
		if GetAmmoInPedWeapon(ped, 883325847) < 4500 then
			...
			QBCore.Functions.Progressbar("refuel-car", "Đổ xăng", 10000, false, true, {
				...
				exports['xt-notify']:Alert("THÔNG BÁO", "Ngừng bơm xăng", 5000, 'error')
				
				StopAnimTask(ped, "weapon@w_sp_jerrycan", "fire", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
			end)
		else
			exports['xt-notify']:Alert("THÔNG BÁO","Xe đã đầy xăng", 5000, 'error')
...
RegisterNetEvent('ps-fuel:client:SendMenuToServer', function()
	...
	if HasPedGotWeapon(ped, 883325847) then
		if GetAmmoInPedWeapon(ped, 883325847) ~= 0 then
			...
			exports['xt-notify']:Alert("THÔNG BÁO", "Can xăng đã cạn xăng", 5000, 'error')
...
RegisterNetEvent('ps-fuel:client:RefuelVehicle', function(refillCost)
	...
	if HasPedGotWeapon(ped, 883325847) then
		...
		if ammo <= 40 then
			...
			exports['xt-notify']:Alert("THÔNG BÁO", "Hết xăng trong can", 5000, 'error')
			...
			QBCore.Functions.Progressbar("refuel-car", "Đổ xăng", time, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			...
			if isCloseVeh() then
				if QBCore.Functions.GetPlayerData().money['cash'] <= refillCost then
					...
					QBCore.Functions.Progressbar("refuel-car", "Đổ xăng", time, false, true, {
						...
						exports['xt-notify']:Alert("THÔNG BÁO", "Ngừng bơm xăng", 5000, 'error')
						--QBCore.Functions.Notify("Ngừng bơm xăng", "error")
						StopAnimTask(ped, "amb@world_human_security_shine_torch@male@base", "base", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
						DeleteObject(gasProp)
					end)
				end
